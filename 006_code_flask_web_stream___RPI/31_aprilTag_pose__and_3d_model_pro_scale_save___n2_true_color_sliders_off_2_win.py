...
def main():
    print(f"\n{'='*60}")
    print(f"🚀 APRILTAG 3D MODEL VIEWER (Два окна)")
    print(f"{'='*60}")
    ...
    cv2.resizeWindow(window_controls, 450, 350)
    cv2.moveWindow(window_controls, DISPLAY_WIDTH - 450, 0)
    
    print(f"\n✅ Создано два окна:")
    print(f"   • {window_main} - {DISPLAY_WIDTH}x{DISPLAY_HEIGHT}")
    print(f"   • {window_controls} - 450x350")
    
    # Функция-заглушка для слайдеров
    def nothing(x):
        pass
    
    # Создание слайдеров в окне управления
    cv2.createTrackbar('Scale', window_controls, slider_defaults['Scale'], 100, nothing)
    cv2.createTrackbar('Rot X', window_controls, slider_defaults['Rot X'], 360, nothing)
    cv2.createTrackbar('Rot Y', window_controls, slider_defaults['Rot Y'], 360, nothing)
    cv2.createTrackbar('Rot Z', window_controls, slider_defaults['Rot Z'], 360, nothing)
    cv2.createTrackbar('Offset X', window_controls, slider_defaults['Offset X'], 1000, nothing)
    cv2.createTrackbar('Offset Y', window_controls, slider_defaults['Offset Y'], 1000, nothing)
    cv2.createTrackbar('Offset Z', window_controls, slider_defaults['Offset Z'], 1000, nothing)
    ...
    print(f"🎚️ Слайдеры в окне Controls")
    print(f"\n{'='*60}\n")
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            for button in buttons:
                if button.is_inside(orig_x, orig_y):
                    if button.text == "SAVE":
                        slider_vals = {
                            'Scale': cv2.getTrackbarPos('Scale', window_controls),
                            'Rot X': cv2.getTrackbarPos('Rot X', window_controls),
                            'Rot Y': cv2.getTrackbarPos('Rot Y', window_controls),
                            'Rot Z': cv2.getTrackbarPos('Rot Z', window_controls),
                            'Offset X': cv2.getTrackbarPos('Offset X', window_controls),
                            'Offset Y': cv2.getTrackbarPos('Offset Y', window_controls),
                            'Offset Z': cv2.getTrackbarPos('Offset Z', window_controls),
                        ...
                        if params:
                            ...
                            cv2.setTrackbarPos('Scale', window_controls, slider_vals['Scale'])
                            cv2.setTrackbarPos('Rot X', window_controls, slider_vals['Rot X'])
                            cv2.setTrackbarPos('Rot Y', window_controls, slider_vals['Rot Y'])
                            cv2.setTrackbarPos('Rot Z', window_controls, slider_vals['Rot Z'])
                            cv2.setTrackbarPos('Offset X', window_controls, slider_vals['Offset X'])
                            cv2.setTrackbarPos('Offset Y', window_controls, slider_vals['Offset Y'])
                            cv2.setTrackbarPos('Offset Z', window_controls, slider_vals['Offset Z'])
                            cv2.setTrackbarPos('Mode: 0pts/1wire/2face', window_controls, slider_vals['Mode'])
                            
                    elif button.text == "RESET":
                        cv2.setTrackbarPos('Scale', window_controls, 50)
                        cv2.setTrackbarPos('Rot X', window_controls, 180)
                        cv2.setTrackbarPos('Rot Y', window_controls, 180)
                        cv2.setTrackbarPos('Rot Z', window_controls, 180)
                        cv2.setTrackbarPos('Offset X', window_controls, 500)
                        cv2.setTrackbarPos('Offset Y', window_controls, 500)
                        cv2.setTrackbarPos('Offset Z', window_controls, 500)
                        print("\n🔄 Слайдеры сброшены")
    ...
    try:
        while True:
            ...
            controls_frame = np.zeros((350, 450, 3), dtype=np.uint8)
            cv2.putText(controls_frame, "CONTROL PANEL", (20, 30), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(controls_frame, "Use sliders above", (20, 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
            cv2.putText(controls_frame, "to adjust model position", (20, 80), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (200, 200, 200), 1)
            cv2.putText(controls_frame, f"Target ID: {TARGET_MARKER_ID}", (20, 120), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 1)
            cv2.putText(controls_frame, f"Detected: {'YES' if marker_detected else 'NO'}", (20, 140), 
