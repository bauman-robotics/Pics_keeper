...
def parse_qr_data(data):
    ...
    try:
        ...
        if unit == 'mm':
            ...
            if value == 25:
                return QR_SIZE  # жестко для 25mm
            elif value == 42:
                return 0.042  # 42 мм (реальный размер)
            else:
                # Если число совпадает с реальным размером
                return value / 1000.0                
...
if CAMERA_TYPE.lower() == 'usb':
    ...
    try:
        ...
        # Явно указываем BGR формат и добавляем управление автофокусом
        ...
        print(f"   Формат: BGR888, Автофокус: ВКЛЮЧЕН")
...
while True:
    ...
    if retval and points is not None:
        # Проходим по каждому обнаруженному QR-коду
        for idx, data in enumerate(decoded_info):
            if data:  # если код распознан
                marker_size = parse_qr_data(data)
                if marker_size:
                    # Получаем точки текущего QR-кода
                    current_points = points[idx].reshape(-1, 2).astype(np.float32)
                    
                    # Рисуем контур для текущего QR-кода
                    for j in range(4):
                        pt1 = tuple(current_points[j].astype(int))
                        pt2 = tuple(current_points[(j+1)%4].astype(int))
                        cv2.line(frame, pt1, pt2, (0, 255, 0), 3)
                    
                    # Решаем PnP для текущего QR-кода
                    ...
                    if success:
                        ...
                        normal_towards_camera = diagnose_orientation(frame, camera_matrix, dist_coeffs, rvec, tvec, marker_size)
                        
                        line_spacing = 35  # базовый интервал между строками

                        y_offset = 30 + idx * 180  # увеличили отступ между QR-кодами

                        cv2.putText(frame, f"QR{idx+1}: {data}", (10, y_offset),
    ...
    # Масштабируем окно под размер экрана (например, 0.8 от исходного размера)
    scale_percent = WIN_SIZE_PERSENT  # 80% от исходного размера
    ...
    cv2.imshow('QR Pose Estimation', frame_display)
