...
class VideoCaptureTester:
    ...
    def test_resolution(self, backend, width, height, target_fps, duration=10):
        """
        Тестирование с поддержкой режима только статистики и выводом в Гбит/сек
        """
        ...
        try:
            ...
            try:
                while time.time() - start_time < duration:
                    ...
                    if ret:
                        ...
                        if current_time - last_log_time >= 2.0:
                            ...
                            # Плотность потока в разных единицах
                            ...
                            print(f"  {mode_indicator} Секунда {int(elapsed)}: {current_fps:.2f} fps | {mb_per_sec:.2f} МБ/с ({gbits_per_sec:.2f} Гбит/с) | кадров: {frame_count}")
                ...
                print(f"  Плотность потока: {mb_per_sec:.2f} МБ/сек | {gbits_per_sec:.2f} Гбит/сек")
                print(f"  Всего данных: {bytes_processed/(1024*1024):.2f} MB ({bytes_processed/(1000*1000*1000)*8:.2f} Гбит)")
                ...
                if not STATS_ONLY_MODE and filename and os.path.exists(filename):
                    ...
                    file_size_gbit = os.path.getsize(filename) * 8 / (1000 * 1000 * 1000)
                    print(f"  Размер файла: {file_size:.2f} MB ({file_size_gbit:.2f} Гбит)")
                    compression_ratio = (1 - file_size/(bytes_processed/(1024*1024)))*100
