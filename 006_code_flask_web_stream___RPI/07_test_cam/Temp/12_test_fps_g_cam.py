...
class VideoCaptureTester:
    ...
    def test_resolution(self, backend, width, height, target_fps, duration=10):
        ...
        test_name = f"{backend}_{width}x{height}_{target_fps}fps"
        ...
        try:
            ...
            temp_filename = filename + ".temp.avi"  # Временный файл для захвата
            ...
            bytes_written = 0
            frames = []  # Сохраняем кадры в памяти
            ...
            try:
                while time.time() - start_time < duration:
                    ...
                    if ret:
                        frame_count += 1
                        frames.append(frame.copy())  # Сохраняем копию кадра
                        bytes_written += frame.nbytes
                        ...
                        if current_time - last_log_time >= 2.0:
                            ...
                            print(f"  Секунда {int(elapsed)}: {current_fps:.2f} fps | {mb_per_sec:.2f} МБ/с | кадров: {frame_count}")
                ...
                print(f"  Время захвата: {total_time:.2f} сек")
                print(f"  Реальный FPS: {actual_fps:.2f}")
                
                # Теперь создаем видеофайл с ПРАВИЛЬНЫМ FPS в заголовке
                if frame_count > 0:
                    print(f"\n📝 Создание видеофайла с FPS = {actual_fps:.2f} в заголовке...")
                    
                    # Создаем VideoWriter с реальным FPS
                    ...
                    if not writer.isOpened():
                        ...
                        # Записываем все сохраненные кадры
                        for i, frame in enumerate(frames):
                            writer.write(frame)
                            if (i + 1) % 50 == 0:
                                print(f"  Записано кадров: {i + 1}/{frame_count}")
                        
                        writer.release()
                        print(f"✅ Видеофайл сохранен: {filename}")
                        ...
                        try:
                            result = subprocess.run(
                                ...
                                '-of', 'default=noprint_wrappers=1:nokey=0', filename],
                                capture_output=True,
                                text=True
                            )
                            print(f"\n🔍 Проверка ffprobe:")
                            ...
                            # Извлекаем длительность из вывода ffprobe
                            for line in result.stdout.split('\n'):
                                if line.startswith('duration='):
                                    ff_duration = float(line.split('=')[1])
                                    print(f"\n✅ Длительность в файле: {ff_duration:.2f} сек")
                                    print(f"   Ожидаемая длительность: {total_time:.2f} сек")
                                    if abs(ff_duration - total_time) < 0.1:
                                        print("   ✓ Совпадает!")
                                    else:
                                        print("   ⚠️ НЕ совпадает!")
                        except Exception as e:
                            print(f"❌ Ошибка проверки ffprobe: {e}")
                
                # Плотность потока
                mb_per_sec = (bytes_written / (1024 * 1024)) / total_time if total_time > 0 else 0
                
                print(f"\n📊 ИТОГИ ТЕСТА:")
                print(f"  Всего кадров: {frame_count}")
                print(f"  Время записи: {total_time:.2f} сек")
                print(f"  Средний FPS захвата: {actual_fps:.2f}")
                print(f"  FPS в заголовке видео: {actual_fps:.2f} (исправлено)")
