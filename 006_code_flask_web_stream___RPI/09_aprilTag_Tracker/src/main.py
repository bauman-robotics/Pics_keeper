...
class AprilTagTracker:
    ...
    def __init__(self, config_path: str = "config/config.yaml"):
        ...
        self.visual_debugger = VisualDebugger(self.config)        
    ...
    def _process_normal_mode(self, frame):
        ...
        if self.marker_detected:
            ...
            self.visual_debugger.print_console_debug(
                self.camera, target_rvec, target_tvec, target_corners
            )
            ...
            # Получаем проекции осей
            img_points = self.visual_debugger.get_axis_projections(
                target_rvec, target_tvec,
                self.camera.camera_matrix, self.camera.dist_coeffs
            )
            
            # Рисуем оси
            x_end, y_end, z_end = self.visual_debugger.draw_axes(frame, img_points)
            
            # Отладочный текст на экране
            self.visual_debugger.draw_debug_text(
                frame, img_points[0], x_end, y_end, z_end
            )
            
            # Рисуем плоскость маркера
            center_point = self.visual_debugger.draw_marker_plane(
                frame, target_rvec, target_tvec,
                self.camera.camera_matrix, self.camera.dist_coeffs
            )
            ...
            if center_point is not None:
                self.visual_debugger.draw_normal(frame, center_point, z_end)
            
            # Логирование в файл
            self.visual_debugger.log_axes_data(
                target_rvec, target_tvec, target_corners, img_points
            )
