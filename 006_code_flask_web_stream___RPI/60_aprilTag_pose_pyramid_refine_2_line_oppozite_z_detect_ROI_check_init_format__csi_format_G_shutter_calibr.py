...
# Форматы CSI камеры:
#   "RGB888"  - стандартный, без сжатия (3 байта на пиксель)
#   "YUV420"  - сжатый сырой формат (1.5 байта на пиксель)
#   "MJPEG"   - аппаратное сжатие JPEG (если поддерживается камерой)
#   "H264"    - аппаратное сжатие H.264 (требуется распаковка)
# --- другие форматы пока не проболвал, скорее не работают ----

CSI_FORMAT = "RGB888"  # По умолчанию стандартный формат


# imx708 Для максимальной производительности с детектированием
# CAMERA_WIDTH = 1536
# CAMERA_HEIGHT = 864
CAMERA_FPS = 120
#CSI_FORMAT = "YUV420"  # Меньше данных для обработки
CSI_BUFFERS = 6        # Больше буферов для плавности

# Целевой FPS для CSI камеры
CSI_FPS_TARGET = 120
# ============================================================================
...
CALIB_PATH = '08_calibration_results_g_shutter_IR'
CAMERA_MATRIX_FILE = f'{CALIB_PATH}/camera_matrix.npy'
DIST_COEFFS_FILE = f'{CALIB_PATH}/dist_coeffs.npy'
...
WINDOWS_CONTROL_EN = True 

# ===== НАСТРОЙКИ КНОПОК =====
MODEL_DEFAULT_VISIBLE = False   # True - модель видна при запуске, False - скрыта
VIDEO_ONLY_DEFAULT = True # False     # True - режим VIDEO ONLY включен при запуске, False - выключен
REFINE_DEFAULT = False         # True - уточнение включено при запуске, False - выключено
# ============================================================================
...
def compute_roi_from_symmetry_lines(frame_shape, face_vertices_2d, center_proj,
    ...
    roi_h_size = min(abs(h_max - h_min) * 0.5, base_size * tangent_scale)  # было 0.4
    roi_v_size = min(abs(v_max - v_min) * 0.8, base_size * normal_scale)   # было 0.6
...
# ============================================================================
# ============================================================================
def draw_rotated_roi_from_info(debug_frame, roi_info, color):
    """
    Рисует повернутый ROI на основе информации из roi_info.
    
    Что рисует:
    - 🔲 Повернутый прямоугольник (4 угла соединенные линиями)
    - 🟡 Центральную точку ROI
    """
    corners = roi_info['corners']  # 4 угла повернутого прямоугольника
    
    # Замыкаем прямоугольник (соединяем последнюю точку с первой)
    closed_corners = np.vstack([corners, corners[0]])
    ...
    # Рисуем центр
    center = roi_info['center']
    cv2.circle(debug_frame, (int(center[0]), int(center[1])), 4, color, -1)

def detect_pyramid_corners(frame_gray, rvec, tvec, camera_matrix, dist_coeffs):
    """
    Детектирует центры граней по черным эллипсам на белом фоне.
    
    Параметры:
        frame_gray - черно-белое изображение
        rvec - вектор поворота маркера
        tvec - вектор перемещения маркера
        camera_matrix - матрица камеры
        dist_coeffs - коэффициенты дисторсии
    
    Возвращает:
        results - список результатов для каждой грани
        Если DEBUG_PYRAMID=True, также возвращает debug_frame
    """
    ...
    # Размеры пирамиды
    half_top = PYRAMID_TOP_SIZE / 2.0 / 1000.0  # 12 мм
    half_base = PYRAMID_BASE_SIZE / 2.0 / 1000.0  # 20 мм
    h_pyr = PYRAMID_HEIGHT / 1000.0  # 8 мм
    
    # Центры граней (на боковых гранях, на половине высоты)
    face_centers_3d = {
    ...
    for face_idx, face in enumerate(PYRAMID_FACES):
        ...
        result_base = {
            ...
            'visible': visible,
            'confidence': 0.0,
            'debug_info': {} if DEBUG_PYRAMID else None
        }
        ...
        if DEBUG_PYRAMID:
            ...
            roi_coords, roi_info = compute_roi_from_symmetry_lines(
                frame_gray.shape, vertices_2d, face_center,
                h_line_start, h_line_end, v_line_start, v_line_end
            )
            draw_rotated_roi_from_info(debug_frame, roi_info, color)
        ...
        # ========== ДОБАВИТЬ ЭТОТ БЛОК ==========
        if DEBUG_PYRAMID:
            # Сохраняем ROI для анализа
            cv2.imwrite(f'debug_roi_{name}_{face_idx}.png', roi)
            print(f"   ROI {name}: форма {roi.shape}, средняя яркость {np.mean(roi):.1f}")
            print(f"   ROI диапазон: min={np.min(roi)}, max={np.max(roi)}")
        # ========================================
                
        if DEBUG_PYRAMID:
            # Показываем ROI в отдельном окне
            roi_display = cv2.resize(roi, (200, 200))
            cv2.imshow(f'ROI {name}', roi_display)
            cv2.waitKey(1)
        ...
        # ========== ДЕТЕКЦИЯ ЭЛЛИПСА ==========
        ellipse_center, ellipse_axes, ellipse_angle, confidence = detect_ellipse_center(roi)
        
        if ellipse_center is None:
            if DEBUG_PYRAMID:
                print(f"   ❌ Эллипс не найден")
            results.append(result_base)
            continue
        
        # Переводим в глобальные координаты
        found_center = ellipse_center + np.array([x1, y1])
        reproj_err = float(np.linalg.norm(found_center - center_proj))
        
        if DEBUG_PYRAMID:
            print(f"   ✅ Эллипс найден:")
            print(f"      центр: ({found_center[0]:.1f}, {found_center[1]:.1f})")
            print(f"      оси: {ellipse_axes[0]:.1f}x{ellipse_axes[1]:.1f}px")
            print(f"      угол: {ellipse_angle:.1f}°")
            print(f"      ошибка: {reproj_err:.1f}px")
            print(f"      уверенность: {confidence:.2f}")
            
            # Рисуем детектированный эллипс
            draw_detected_ellipse(debug_frame, found_center, ellipse_axes, ellipse_angle, color)
            
            # Рисуем линию от проекции к найденному центру
            cv2.line(debug_frame, 
                    (int(center_proj[0]), int(center_proj[1])),
                    (int(found_center[0]), int(found_center[1])),
                    (255, 255, 255), 1)
        ...
        if DEBUG_PYRAMID:
            result['debug_info'] = {
                ...
                'face_vertices': vertices_2d,
                'face_bounds': (np.min(vertices_2d[:,0]), np.min(vertices_2d[:,1]),
                               np.max(vertices_2d[:,0]), np.max(vertices_2d[:,1]))
            }
...
class Button:
    def __init__(self, x, y, width, height, text,
                 ...
                 toggle=False, active_color=None, active=False):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
        ...
        # ОТЛАДКА: показываем, с какими параметрами создаётся кнопка
        # print(f"🔧 Button CREATED: {text}")
        # print(f"   toggle={toggle}, active_color={active_color}, active={active}")
        # print(f"   self.active={self.active}, self.active_color={self.active_color}")

    def draw(self, frame):
        # Определяем цвет в зависимости от состояния
        if self.toggle and self.active:
            # Активная кнопка-переключатель - яркий цвет
            if self.active_color:
                color = self.active_color
            else:
                color = (0, 255, 0)  # Ярко-зелёный по умолчанию
        else:
            # Неактивная кнопка - более яркий серый/цветной
            if self.is_hovered:
...
def init_camera():
    ...
    if CAMERA_TYPE == 'usb':
        ...
        if not ret or test_frame is None:
            print("❌ CRITICAL: Cannot get frame from camera!")
            sys.exit(1)
        else:
        ...
        print(f"  Режим: CSI камера (picamera2), формат: {CSI_FORMAT}")
        
        from picamera2 import Picamera2
        picam2 = Picamera2(0)
        
        # Настройка в зависимости от выбранного формата
        if CSI_FORMAT == "RGB888":
            print("   📷 Формат: RGB888 (без сжатия, 3 байта/пиксель)")
            main_format = "RGB888"
            
        elif CSI_FORMAT == "YUV420":
            print("   📷 Формат: YUV420 (сжатый сырой, 1.5 байта/пиксель)")
            main_format = "YUV420"
            
        elif CSI_FORMAT == "MJPEG":
            print("   📷 Формат: MJPEG (аппаратное сжатие JPEG)")
            main_format = "MJPEG"
            
        elif CSI_FORMAT == "H264":
            print("   📷 Формат: H264 (аппаратное сжатие видео)")
            # Для H264 нужна специальная конфигурация
            config = picam2.create_video_configuration(
                main={"size": (CAMERA_WIDTH, CAMERA_HEIGHT), "format": "H264"},
                controls={"FrameRate": CSI_FPS_TARGET, "AfMode": 2},
                buffer_count=CSI_BUFFERS
            )
            picam2.configure(config)
            picam2.start()
            time.sleep(1)
            
            # Получаем тестовый кадр
            test_frame = picam2.capture_array()
            width, height = test_frame.shape[1], test_frame.shape[0]
            CAMERA_WIDTH, CAMERA_HEIGHT = width, height
            cap = None
            print(f"✅ CSI camera: {width}x{height} @ {CSI_FPS_TARGET}fps (H264)")
            print(f"✅ Test frame captured: {test_frame.shape}")
            
            # Возвращаемся, чтобы не выполнять общую часть
        ...
        config = picam2.create_video_configuration(
            main={"size": (CAMERA_WIDTH, CAMERA_HEIGHT), "format": main_format},
            controls={"FrameRate": CSI_FPS_TARGET, "AfMode": 2},
            buffer_count=4
        )
        ...
        print(f"✅ CSI camera: {width}x{height} @ {actual_fps:.1f}fps ({CSI_FORMAT})")
        print(f"✅ Test frame captured: {test_frame.shape}")
        print(f"   Байт на пиксель: {test_frame.nbytes / (width * height):.1f}")
        print(f"   Размер кадра: {test_frame.nbytes / 1024:.1f} КБ")
...
# ============================================================================
# ОТРИСОВКА Кругов на гранях
# ============================================================================
def draw_detected_ellipse(debug_frame, ellipse_center, ellipse_axes, ellipse_angle, color):
    """
    Рисует детектированный эллипс на отладочном кадре.
    """
    # Рисуем контур эллипса
    cv2.ellipse(debug_frame, 
                (int(ellipse_center[0]), int(ellipse_center[1])),
                (int(ellipse_axes[0]/2), int(ellipse_axes[1]/2)),
                ellipse_angle, 0, 360, color, 1, cv2.LINE_AA)
    
    # Рисуем центр эллипса
    ...
    dx = int(ellipse_axes[0]/2 * np.cos(angle_rad))
    dy = int(ellipse_axes[0]/2 * np.sin(angle_rad))
    cv2.line(debug_frame,
             (int(ellipse_center[0]) - dx, int(ellipse_center[1]) - dy),
             (int(ellipse_center[0]) + dx, int(ellipse_center[1]) + dy),
             color, 1, cv2.LINE_AA)
    
    dx = int(ellipse_axes[1]/2 * np.cos(angle_rad + np.pi/2))
    dy = int(ellipse_axes[1]/2 * np.sin(angle_rad + np.pi/2))
    cv2.line(debug_frame,
             (int(ellipse_center[0]) - dx, int(ellipse_center[1]) - dy),
             (int(ellipse_center[0]) + dx, int(ellipse_center[1]) + dy),
             color, 1, cv2.LINE_AA)
...
def detect_ellipse_center(roi):
    """
    Детектирует центр черного эллипса на белом фоне.
    Эллипс должен полностью помещаться в ROI.
    """
    if roi.size == 0:
    ...
    max_dist = min(h, w) * 0.4  # максимальное расстояние от центра
    ...
    for contour in contours:
        ...
        # Фильтр по площади (10-40% от ROI)
        roi_area = h * w
        ...
        if len(contour) >= 5:
            ...
            # ПРОВЕРКА: центр эллипса должен быть близко к центру ROI
            dist_from_center = np.linalg.norm(center - roi_center)
            if dist_from_center > max_dist:
                continue
            
            # ПРОВЕРКА: эллипс должен полностью помещаться в ROI
            # Создаем маску эллипса
            ...
            # Проверяем, не выходит ли эллипс за границы ROI
            # Для этого смотрим пиксели на границе ROI
            ...
            if cv2.countNonZero(ellipse_border) > 0:
                continue  # эллипс касается границы - не подходит
    ...
    if best_ellipse:
        ...
        return center, axes, angle, best_confidence
    
    return None, None, None, 0.0

def draw_pyramid_corners(frame, pyramid_results, rvec, tvec, camera_matrix, dist_coeffs):
    """
    Отрисовывает на кадре найденные центры граней и эллипсы.
    
    Параметры:
        frame - кадр для отрисовки (BGR)
        pyramid_results - список результатов от detect_pyramid_corners
        rvec, tvec - поза маркера (для отрисовки ребер)
    ...
    # Рисуем ребра пирамиды для контекста
    draw_pyramid_edges(frame, rvec, tvec, camera_matrix, dist_coeffs)
    ...
    for r in pyramid_results:
        ...
        # ===== 1. Рисуем проекцию центра грани (теоретическое положение) =====
        if 'proj_center' in r and r['proj_center'] is not None:
            px, py = int(r['proj_center'][0]), int(r['proj_center'][1])
            
            # Проверяем, что точка в пределах кадра
            if 0 <= px < frame.shape[1] and 0 <= py < frame.shape[0]:
                # Рисуем проекцию (круг с крестом)
                cv2.circle(frame, (px, py), 6, color, 1)
                cv2.drawMarker(frame, (px, py), color,
                              cv2.MARKER_CROSS, 12, 1, cv2.LINE_AA)
                
                # Подпись грани
                cv2.putText(frame, f"{name}_proj", (px+10, py-5),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)
        
        # ===== 2. Рисуем найденный центр грани =====
        if 'center_2d' in r and r['center_2d'] is not None:
            ...
            if 0 <= cx < frame.shape[1] and 0 <= cy < frame.shape[0]:
                # Найденный центр - большой зеленый круг
                cv2.circle(frame, (cx, cy), 8, (0, 255, 0), -1)
                cv2.circle(frame, (cx, cy), 10, (255, 255, 255), 1)
                
                # Линия от проекции к найденному центру
                if 'proj_center' in r and r['proj_center'] is not None:
                    px, py = int(r['proj_center'][0]), int(r['proj_center'][1])
                    if 0 <= px < frame.shape[1] and 0 <= py < frame.shape[0]:
                        cv2.line(frame, (px, py), (cx, cy), (255, 255, 255), 1)
                
                # Показываем ошибку и уверенность
                info_text = []
                if 'reproj_err' in r:
                    info_text.append(f"err:{r['reproj_err']:.1f}px")
                if 'confidence' in r:
                    info_text.append(f"conf:{r['confidence']:.2f}")
                
                if info_text:
                    cv2.putText(frame, " ".join(info_text), 
        ...
        # ===== 3. Рисуем дополнительную информацию из отладки =====
        if 'debug_info' in r and r['debug_info'] is not None:
            debug = r['debug_info']
            
            # Рисуем bounding box грани если есть
            if 'face_bounds' in debug:
                x_min, y_min, x_max, y_max = debug['face_bounds']
                cv2.rectangle(frame, 
                             (int(x_min), int(y_min)),
                             (int(x_max), int(y_max)),
                             color, 1)
            
            # Рисуем вершины грани если есть
            if 'face_vertices' in debug:
                vertices = debug['face_vertices']
...
def main():
    ...
    def mouse_callback(event, x, y, flags, param):
        nonlocal fullscreen_mode, auto_center_requested, mouse_x, mouse_y
        ...
        # Конвертируем координаты экрана в координаты исходного кадра
        ...
        # Обновляем hover-состояние для всех кнопок
        for btn in _buttons:
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            for btn in _buttons:
                if btn.is_inside(orig_x, orig_y):
                    print(f"\n🔘 Button clicked: {btn.text}")
                    
                    if btn.text == "VIDEO ONLY":
                        btn.active = not btn.active
                        print(f"   VIDEO ONLY: {'ON' if btn.active else 'OFF'}")
                    ...
                    break  # Важно: выходим после обработки нажатия
    ...
    def get_btn(name):
        for b in buttons:
            if b.text == name:
                return b
        return None
    btn_video_only = get_btn("VIDEO ONLY")  # Новая кнопка
    ...
    try:
        while True:
            ...
            if video_only_mode:
                ...
                # Минимальная информация на экране
                tag_info = "VIDEO ONLY MODE"
                
                # Получаем параметры слайдеров для совместимости
                slider_vals = _get_all_sliders()
                params = slider_to_params(slider_vals)
                
            else:
                # Нормальный режим со всей обработкой
                ...
                for detection in detections:
                    ...
                    if detection.tag_id == TARGET_MARKER_ID:
                        target_rvec = rvec
                        target_tvec = tvec
                        target_img_corners = np.array(detection.corners, dtype=np.float32)
                ...
                # СЛАЙДЕРЫ (только для нормального режима)
                slider_vals = _get_all_sliders()
                params = slider_to_params(slider_vals)
            ...
            # ===== ЭТОТ КОД ВЫПОЛНЯЕТСЯ ВСЕГДА (и для VIDEO ONLY, и для NORMAL) =====
            
            # КНОПКИ (всегда рисуем)
            for btn in buttons:
                btn.draw(frame)
            ...
            # КНОПКИ
            # ================================================================
            # print(f"Рисуем {len(buttons)} кнопок")  # Временная отладка
            # for btn in buttons:
            #     btn.draw(frame)
            #     print(f"  Нарисована кнопка {btn.text} на позиции ({btn.x}, {btn.y})")  # Временная отладка                

            # ИНФОРМАЦИЯ НА КАДРЕ
            refine_str = ""
            if not video_only_mode and 'refine_active' in locals() and refine_active:
            ...
            info_lines = [
                f"FPS: {fps:.1f}  |  Mode: {'VIDEO ONLY' if video_only_mode else 'NORMAL'}",
            ...
            # Статус VIDEO ONLY
            video_only_color = (0,255,0) if video_only_mode else (100,100,100)
            cv2.putText(controls_frame,
                        f"VIDEO ONLY: {'ACTIVE' if video_only_mode else 'OFF'}",
                        (20, 450), cv2.FONT_HERSHEY_SIMPLEX, 0.5, video_only_color, 1)

            # Статус REFINE
            refine_color = (0,220,255) if not video_only_mode and refine_active else (120,120,120)
            cv2.putText(controls_frame,
                        f"Line cross refine: {'ACTIVE' if not video_only_mode and refine_active else 'OFF'}",
                        (20, 480), cv2.FONT_HERSHEY_SIMPLEX, 0.5, refine_color, 1)
            
            if not video_only_mode and refine_active:
                cv2.putText(controls_frame,
                            f"Line crosses found: {n_pyramid_found}/4", (20, 500),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200,200,200), 1)
                rb, ra = refine_reproj_info
                if rb is not None:
                    cv2.putText(controls_frame,
                                ...
                                (20, 520), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200,200,200), 1)

            cv2.putText(controls_frame, "Current fine values:", (20, 550),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,255,255), 1)
            cv2.putText(controls_frame,
                        f"Scale fine: {(slider_vals['Scale_fine']-500)/500*10:+.1f}%",
                        (20, 570), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200,200,200), 1)
            cv2.putText(controls_frame,
                        ...
                        (20, 590), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (200,200,200), 1)
            cv2.putText(controls_frame,
                        f"Offset fine:"
