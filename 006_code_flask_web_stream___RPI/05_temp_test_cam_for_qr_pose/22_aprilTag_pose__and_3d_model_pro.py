...
def params_to_slider_values(params):
    ...
    # Scale: от 1.0 до 2.0 (масштаб)
    if 'scale' in params:
        slider_values['Scale'] = 0.001 #int((params['scale'] - 1.0) * 100)  # 1.0 -> 0, 2.0 -> 100
        
    # Rotations: от -180 до 180 -> от 0 до 360
    for axis in ['rot_x', 'rot_y', 'rot_z']:
        if axis in params:
            slider_values[axis.replace('rot_', 'Rot ').upper()] = int(params[axis] + 180)
    
    # Offsets: от -50 до 50 -> от 0 до 1000
    for axis, default in [('offset_x', 0), ('offset_y', 0), ('offset_z', 0)]:
        if axis in params:
            slider_values[axis.replace('offset_', 'Offset ').upper()] = int((params[axis] + 50) * 10)
...
def slider_values_to_params(slider_values):
    ...
    # Scale: от 0 до 100 -> от 1.0 до 2.0
    if 'Scale' in slider_values:
        params['scale'] = 1.0 + slider_values['Scale'] / 100.0
    ...
    for slider_name, param_name in [('Rot X', 'rot_x'), ('Rot Y', 'rot_y'), ('Rot Z', 'rot_z')]:
        if slider_name in slider_values:
            params[param_name] = slider_values[slider_name] - 180
