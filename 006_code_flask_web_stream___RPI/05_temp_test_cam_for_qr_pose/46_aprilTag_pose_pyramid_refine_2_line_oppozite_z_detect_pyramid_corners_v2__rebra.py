...
def detect_pyramid_corners(frame_gray, rvec, tvec, camera_matrix, dist_coeffs):
    """
    Детектирует углы пирамиды. Если грань слишком маленькая,
    используем увеличенное ROI и шаблонный поиск.
    """
    ...
    if DEBUG_PYRAMID:
        ...
        print(f"\n{'='*60}")
        print(f"🔍 PYRAMID CORNER DETECTION DEBUG (SMALL OBJECT) - Frame {h}x{w}")
        print(f"{'='*60}")
    ...
    for face_idx, face in enumerate(PYRAMID_FACES):
        ...
        # ========== ОПРЕДЕЛЯЕМ 4 УГЛА ГРАНИ ==========
        if name == 'FRONT':
            corners_3d = [
                np.array([-half_top, -half_top, 0.0]),  # Левый верхний
                np.array([ half_top, -half_top, 0.0]),  # Правый верхний
                np.array([ half_base, -half_base, h_pyr]),  # Правый нижний
                np.array([-half_base, -half_base, h_pyr])   # Левый нижний
            ]
        elif name == 'BACK':
            corners_3d = [
                np.array([ half_top, half_top, 0.0]),   # Правый верхний
            ...
            corners_3d = [
                np.array([-half_top, -half_top, 0.0]),  # Передний верхний
                np.array([-half_top, half_top, 0.0]),   # Задний верхний
                np.array([-half_base, half_base, h_pyr]), # Задний нижний
                np.array([-half_base, -half_base, h_pyr])  # Передний нижний
            ]
        else:  # RIGHT
            corners_3d = [
                np.array([ half_top, half_top, 0.0]),   # Задний верхний
        ...
        for corner_3d in corners_3d:
            ...
            proj, _ = cv2.projectPoints(corner_m, rvec, tvec, camera_matrix, dist_coeffs)
            corners_2d.append(proj[0][0])
        
        corners_2d = np.array(corners_2d)
        
        # Определяем целевой угол (верхний левый для каждой грани)
        if name == 'FRONT':
            target_idx = 0  # Левый верхний
        elif name == 'BACK':
            target_idx = 1  # Левый верхний для BACK
        elif name == 'LEFT':
            target_idx = 0  # Передний верхний
        else:  # RIGHT
            target_idx = 1  # Задний верхний
        
        target_corner_3d = corners_3d[target_idx]
        target_corner_2d_proj = corners_2d[target_idx]
        
        if DEBUG_PYRAMID:
            print(f"   📍 Целевой угол ({target_idx}): ({target_corner_2d_proj[0]:.1f}, {target_corner_2d_proj[1]:.1f})")
        ...
        if DEBUG_PYRAMID:
            print(f"   📏 Размер грани: {face_width:.1f}x{face_height:.1f}px")
        
        # Минимальный размер ROI для поиска
        min_roi_size = 60  # пикселей
        
        # Центр грани
        center_x = (min_x + max_x) / 2
        center_y = (min_y + max_y) / 2
        
        # Создаем ROI фиксированного размера вокруг центра грани
        roi_size = max(min_roi_size, int(max(face_width, face_height) * 3))
        
        x1 = max(int(center_x - roi_size/2), 0)
        y1 = max(int(center_y - roi_size/2), 0)
        x2 = min(int(center_x + roi_size/2), w)
        y2 = min(int(center_y + roi_size/2), h)
        ...
        if DEBUG_PYRAMID:
            ...
            # Рисуем проекцию целевого угла
            cv2.circle(debug_frame, (int(target_corner_2d_proj[0]), int(target_corner_2d_proj[1])), 
                      4, (0, 255, 255), -1)

        if roi.size == 0 or roi.shape[0] < 30 or roi.shape[1] < 30:
            empty_result = {
                'face': name,
                'corner_3d': target_corner_3d * 1000.0,
                'corner_2d': None,
                'proj_corner': target_corner_2d_proj,
                'reproj_err': None,
                'visible': visible,
                'confidence': 0.0
            }
        ...
        # ========== УЛУЧШЕННЫЙ ПОИСК ДЛЯ МАЛЕНЬКИХ ОБЪЕКТОВ ==========
        
        # 1. Увеличиваем контраст
        roi_eq = cv2.equalizeHist(roi)
        
        # 2. Детектор углов Shi-Tomasi
        ...
        if corners is not None:
            ...
            if DEBUG_PYRAMID:
                print(f"   🔍 Найдено углов Shi-Tomasi: {len(corners)}")
            
            # Переводим в глобальные координаты
            global_corners = corners + np.array([x1, y1])
            
            # Находим ближайший к проекции целевого угла
            min_dist = float('inf')
            best_corner = None
            
            for corner in global_corners:
                dist = np.sqrt((corner[0] - target_corner_2d_proj[0])**2 + 
                              (corner[1] - target_corner_2d_proj[1])**2)
                if dist < min_dist:
                    min_dist = dist
                    best_corner = corner
            
            if best_corner is not None:
                reproj_err = float(np.linalg.norm(best_corner - target_corner_2d_proj))
                
                if DEBUG_PYRAMID:
                    print(f"   ✅ Найден угол Shi-Tomasi: ({best_corner[0]:.1f}, {best_corner[1]:.1f})")
                    print(f"   📏 Ошибка: {reproj_err:.1f}px")
                    cv2.circle(debug_frame, (int(best_corner[0]), int(best_corner[1])), 
                              6, (0, 255, 0), -1)
                
                # Динамический порог
                max_allowed_error = PYRAMID_REPROJ_THRESHOLD * (3.0 + distance_m * 3)
        ...
        # 3. Если Shi-Tomasi не сработал, пробуем бинаризацию и поиск линий
        # Усиленное размытие
        ...
        lines = cv2.HoughLinesP(
            ...
            theta=np.pi/180,
            threshold=15,
            minLineLength=max(roi.shape[1]//10, 10),
            maxLineGap=5
        )
        
        if lines is None or len(lines) < 3:
            if DEBUG_PYRAMID:
                print(f"   ❌ Недостаточно линий")
            empty_result = {
                'face': name,
                'corner_3d': target_corner_3d * 1000.0,
                'corner_2d': None,
        ...
        for line in lines[:, 0]:
            ...
            if angle < 20 or angle > 160:
                horizontal.append((x1_l, y1_l, x2_l, y2_l))
            elif 70 < angle < 110:
                vertical.append((x1_l, y1_l, x2_l, y2_l))
        
        if len(horizontal) < 1 or len(vertical) < 1:
            if DEBUG_PYRAMID:
                print(f"   ❌ Недостаточно ориентированных линий")
            empty_result = {
                'face': name,
                'corner_3d': target_corner_3d * 1000.0,
                'corner_2d': None,
        ...
        # Находим пересечения
        intersections = []
        for h_line in horizontal[:3]:
            for v_line in vertical[:3]:
                ...
                if 0 <= x < roi.shape[1] and 0 <= y < roi.shape[0]:
                    intersections.append((x + x1, y + y1))
        ...
        # Выбираем ближайшее к проекции
        min_dist = float('inf')
        best_corner = None
        for pt in intersections:
            dist = np.sqrt((pt[0] - target_corner_2d_proj[0])**2 + 
                          (pt[1] - target_corner_2d_proj[1])**2)
            if dist < min_dist:
                min_dist = dist
                best_corner = pt
        ...
        confidence = min(1.0, (len(horizontal) + len(vertical)) / 20.0)
    ...
    if DEBUG_PYRAMID:
        ...
        cv2.imshow('Pyramid Debug - Small Object', debug_frame)
        cv2.waitKey(1)
