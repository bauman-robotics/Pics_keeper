...
def draw_face_symmetry_lines(debug_frame, face_vertices_2d, center_proj, color):
    """
    Рисует две линии симметрии для грани, проходящие через ЦЕНТР грани.
    """
    ...
    # Вычисляем направления ребер
    top_edge = top_right - top_left
    top_edge_len = np.linalg.norm(top_edge)
    if top_edge_len > 0:
        top_edge_dir = top_edge / top_edge_len
    else:
        top_edge_dir = np.array([1.0, 0.0])
    
    left_edge = bottom_left - top_left
    left_edge_len = np.linalg.norm(left_edge)
    if left_edge_len > 0:
        left_edge_dir = left_edge / left_edge_len
    else:
        left_edge_dir = np.array([0.0, 1.0])
    
    # ВЫЧИСЛЯЕМ ЦЕНТР ГРАНИ (как среднее всех вершин)
    face_center = np.mean(face_vertices_2d, axis=0)
    
    # Линия, параллельная верхнему ребру, проходящая через ЦЕНТР грани
    # Идем от левого края до правого края грани на уровне центра
    h_line_start = face_center - top_edge_dir * top_edge_len * 0.5
    h_line_end = face_center + top_edge_dir * top_edge_len * 0.5
    
    # Линия, параллельная боковому ребру, проходящая через ЦЕНТР грани
    # Идем от верхнего края до нижнего края грани на уровне центра
    v_line_start = face_center - left_edge_dir * left_edge_len * 0.5
    v_line_end = face_center + left_edge_dir * left_edge_len * 0.5
    
    # Рисуем линию вдоль грани (красную)
    cv2.line(debug_frame, 
             (int(h_line_start[0]), int(h_line_start[1])),
             (int(h_line_end[0]), int(h_line_end[1])),
             (0, 0, 255), 2, cv2.LINE_AA)
    
    # Рисуем линию вниз по грани (синюю)
    cv2.line(debug_frame,
    ...
    # Для сравнения показываем и переданный center_proj (белым)
    cv2.circle(debug_frame, (int(center_proj[0]), int(center_proj[1])), 4, (255, 255, 255), -1)
...
def compute_face_roi(frame_shape, name, marker_corners_2d, face_center_proj, 
                     normal_dir, tangent_dir, rvec, tvec, camera_matrix, dist_coeffs,
                     base_size=70, normal_scale=1.8, tangent_scale=1.0):
    """
    Вычисляет повернутый ROI вокруг проекции центра грани.
    С проверкой правильности направлений.
    """
    ...
    # Вершины грани в зависимости от имени
    if name == 'FRONT':
        ...
        # Для FRONT грани: нормаль должна быть направлена ВНИЗ (к основанию)
        expected_normal_sign = -1.0  # вниз по грани
    elif name == 'BACK':
        vertices_3d = np.array([
            [ half_top, half_top, 0.0],
            [-half_top, half_top, 0.0],
    ...
    # Находим центр грани по вершинам
    face_center_from_vertices = np.mean(vertices_2d, axis=0)
    
    # Определяем вектор от верхнего края к нижнему
    top_edge_center = np.mean(vertices_2d[:2], axis=0)  # верхние две вершины
    bottom_edge_center = np.mean(vertices_2d[2:], axis=0)  # нижние две вершины
    
    true_normal_dir = bottom_edge_center - top_edge_center
    if np.linalg.norm(true_normal_dir) > 0:
        true_normal_dir = true_normal_dir / np.linalg.norm(true_normal_dir)
    
    # Сравниваем с вычисленным normal_dir и корректируем если нужно
    dot_product = np.dot(normal_dir, true_normal_dir)
    
    if dot_product < 0:  # направления противоположны
        normal_dir = -normal_dir
        tangent_dir = -tangent_dir
        if DEBUG_PYRAMID:
            print(f"   🔄 Направление скорректировано (dot={dot_product:.3f})")
    
    # Находим размеры проекции грани
    face_min_x = np.min(vertices_2d[:, 0])
    face_max_x = np.max(vertices_2d[:, 0])
    face_min_y = np.min(vertices_2d[:, 1])
    face_max_y = np.max(vertices_2d[:, 1])
    
    face_width = face_max_x - face_min_x
    face_height = face_max_y - face_min_y
    
    # Вычисляем размеры ROI (не больше размеров грани)
    roi_normal = min(int(base_size * normal_scale), int(face_height * 0.8))
    roi_tangent = min(int(base_size * tangent_scale), int(face_width * 0.8))
    ...
    # Дополнительно ограничиваем углы ROI проекцией грани
    for i in range(4):
        corners[i][0] = np.clip(corners[i][0], face_min_x, face_max_x)
        corners[i][1] = np.clip(corners[i][1], face_min_y, face_max_y)
    ...
    info = {
        ...
        'normal_dir': normal_dir,
        'tangent_dir': tangent_dir,
        'true_normal_dir': true_normal_dir,
        'corners': corners,
        'roi_normal_size': roi_normal,
        'roi_tangent_size': roi_tangent,
        'face_bounds': (face_min_x, face_min_y, face_max_x, face_max_y),
    ...
    # Для выделения ROI используем bounding box
    x1 = max(0, int(np.min(corners[:, 0])))
    y1 = max(0, int(np.min(corners[:, 1])))
    x2 = min(w, int(np.max(corners[:, 0])))
    y2 = min(h, int(np.max(corners[:, 1])))
...
def detect_pyramid_corners(frame_gray, rvec, tvec, camera_matrix, dist_coeffs):
    ...
    for face_idx, face in enumerate(PYRAMID_FACES):
        ...
        if DEBUG_PYRAMID:
            ...
            draw_face_symmetry_lines(debug_frame, vertices_2d, center_proj, color)
