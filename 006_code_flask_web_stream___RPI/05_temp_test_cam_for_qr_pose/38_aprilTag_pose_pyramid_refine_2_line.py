...
def detect_pyramid_corners(frame_gray, rvec, tvec, camera_matrix, dist_coeffs):
    """
    Детектирует субпиксельные точки пересечения вертикальной и горизонтальной линий
    на боковых гранях пирамиды.
    
    Улучшенная версия с валидацией и робастным детектором.
    """
    results = []
    ...
    for face in PYRAMID_FACES:
        ...
        result_base = {
            ...
            'reproj_err': None,
            'visible':    visible,
            'confidence': 0.0,  # Добавляем уровень уверенности
        }
        ...
        if (x2 - x1) < 40 or (y2 - y1) < 40:  # Увеличиваем минимальный размер
            if DEBUG_PYRAMID:
        ...
        if DEBUG_PYRAMID:
            cv2.imwrite(f"debug_roi_{name}_{frame_count}.jpg", roi)

        # --- УЛУЧШЕННЫЙ ПОИСК ПЕРЕСЕЧЕНИЯ ЛИНИЙ ---
        
        # 1. Адаптивная бинаризация (лучше работает при разном освещении)
        ...
        # 2. Морфологическая очистка шума
        ...
        # 3. Выделение линий с адаптивными ядрами
        roi_h, roi_w = roi_binary.shape
        
        # Адаптивный размер ядра в зависимости от размера ROI
        kernel_size = max(roi_w // 10, 5)
        if kernel_size % 2 == 0:
            kernel_size += 1
        ...
        # 5. Анализ найденных пересечений
        y_coords, x_coords = np.where(intersection > 0)
        
        if len(x_coords) < 10:  # Увеличиваем порог
            if DEBUG_PYRAMID:
        ...
        # 6. Проверка распределения точек (должны быть сконцентрированы)
        x_std = np.std(x_coords)
        y_std = np.std(y_coords)
        
        # Если точки слишком разбросаны - низкая уверенность
        max_std = roi_w // 8
        if x_std > max_std or y_std > max_std:
            if DEBUG_PYRAMID:
                print(f"  Face {name}: points too scattered (x_std={x_std:.1f}, y_std={y_std:.1f})")
            results.append(result_base)
            continue
            
        # 7. Проверка на линии (должны быть достаточно длинные)
        # Горизонтальная линия
        h_line_length = np.sum(horizontal_lines[cy_roi.astype(int), :] > 0)
        # Вертикальная линия
        v_line_length = np.sum(vertical_lines[:, cx_roi.astype(int)] > 0)
        
        min_line_length = roi_w // 3
        if h_line_length < min_line_length or v_line_length < min_line_length:
            if DEBUG_PYRAMID:
                print(f"  Face {name}: lines too short (h={h_line_length}, v={v_line_length})")
            results.append(result_base)
            continue
            
        # 8. Уверенность детекции (комбинированный показатель)
        confidence = min(1.0, (len(x_coords) / 100.0) * 
                               (min_line_length / max(h_line_length, 1)) *
                               (1.0 - (x_std + y_std) / (2 * max_std)))
        
        # 9. Субпиксельное уточнение с большим окном
        candidate = np.array([[[cx_roi, cy_roi]]], dtype=np.float32)
        corners_subpix = cv2.cornerSubPix(
            roi,
            candidate,
            winSize=(15, 15),  # Увеличиваем окно
            zeroZone=(-1, -1),
        ...
        # 10. Репроекционная ошибка
        reproj_err = float(np.linalg.norm(corner_full - np.array([px, py])))
        
        # Адаптивный порог в зависимости от расстояния
        adaptive_threshold = PYRAMID_REPROJ_THRESHOLD * (1.0 + distance_m)
        
        if reproj_err > adaptive_threshold:
            if DEBUG_PYRAMID:
                print(f"  Face {name}: reproj_err={reproj_err:.1f}px > {adaptive_threshold:.1f}px")
            results.append(result_base)
            continue
            
        # 11. Проверка, что точка находится в пределах грани
        # Проецируем все 4 угла грани
        face_vertices_3d = []
        for sign_x in [-1, 1]:
            for sign_y in [-1, 1]:
                # Упрощенно: углы грани (приблизительно)
                if name == 'FRONT':
                    vert = np.array([sign_x * PYRAMID_TOP_SIZE/2, 
                                     -PYRAMID_TOP_SIZE/2, 
                                     (1 + sign_y) * PYRAMID_HEIGHT/2])
                elif name == 'BACK':
                    vert = np.array([sign_x * PYRAMID_TOP_SIZE/2, 
                                     PYRAMID_TOP_SIZE/2, 
                                     (1 + sign_y) * PYRAMID_HEIGHT/2])
                elif name == 'LEFT':
                    vert = np.array([-PYRAMID_TOP_SIZE/2, 
                                     sign_x * PYRAMID_TOP_SIZE/2, 
                                     (1 + sign_y) * PYRAMID_HEIGHT/2])
                else:  # RIGHT
                    vert = np.array([PYRAMID_TOP_SIZE/2, 
                                     sign_x * PYRAMID_TOP_SIZE/2, 
                                     (1 + sign_y) * PYRAMID_HEIGHT/2])
                face_vertices_3d.append(vert / 1000.0)
        
        # Проецируем углы в 2D
        face_vertices_2d, _ = cv2.projectPoints(
            np.array(face_vertices_3d, dtype=np.float32),
            rvec, tvec, camera_matrix, dist_coeffs
        )
        face_vertices_2d = face_vertices_2d.reshape(-1, 2)
        
        # Проверяем, находится ли точка внутри bounding box грани
        min_x = np.min(face_vertices_2d[:, 0])
        max_x = np.max(face_vertices_2d[:, 0])
        min_y = np.min(face_vertices_2d[:, 1])
        max_y = np.max(face_vertices_2d[:, 1])
        
        margin = 20  # пикселей допуска
        if (corner_full[0] < min_x - margin or 
            corner_full[0] > max_x + margin or
            corner_full[1] < min_y - margin or 
            corner_full[1] > max_y + margin):
            if DEBUG_PYRAMID:
                print(f"  Face {name}: point outside face bounds")
            results.append(result_base)
            continue
        ...
        if DEBUG_PYRAMID:
            print(f"  Face {name}: FOUND subpix=({corner_full[0]:.1f},{corner_full[1]:.1f}), "
                  f"reproj_err={reproj_err:.1f}px, confidence={confidence:.2f}")
