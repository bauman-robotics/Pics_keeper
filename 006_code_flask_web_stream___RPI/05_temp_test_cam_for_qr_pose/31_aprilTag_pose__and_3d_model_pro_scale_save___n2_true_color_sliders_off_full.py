...
try:
    ...
    # Если tkinter не доступен, используем стандартное разрешение
    DISPLAY_WIDTH = 1280
    DISPLAY_HEIGHT = 720
...
def get_frame():
    ...
    if CAMERA_TYPE == 'usb':
        ...
        if not ret:
            return None
        # USB камера: обычно BGR
        return frame
    else:  # CSI
        frame = picam2.capture_array()
        # CSI камера: нужно проверить формат!
        # Если цвета инвертированы (синий вместо желтого), значит frame уже BGR
        return frame  # Пробуем без конвертации


# Функция масштабирования:
def resize_frame_to_display(frame, target_width, target_height):
    """Масштабирует кадр с сохранением пропорций и возвращает масштабированный кадр и коэффициенты"""
    h, w = frame.shape[:2]
    ...
    # Защита от нулевых новых размеров
    new_w = max(1, new_w)
    new_h = max(1, new_h)
    ...
    # Вычисляем коэффициенты преобразования координат
    scale_x = new_w / w
    scale_y = new_h / h
...
def main():
    ...
    fullscreen_mode = False  # Режим отображения: False - со слайдерами, True - полноэкранный
    ...
    # Создаем окно с нужным размером СРАЗУ
    ...
    try:
        ...
        print(f"✅ Окно создано: {rect[2]}x{rect[3]} в позиции ({rect[0]}, {rect[1]})")
    except:
        print(f"✅ Окно создано: {DISPLAY_WIDTH}x{DISPLAY_HEIGHT}")
    ...
    current_slider_values = {
        'Scale': slider_defaults['Scale'],
        'Rot X': slider_defaults['Rot X'],
        'Rot Y': slider_defaults['Rot Y'],
        'Rot Z': slider_defaults['Rot Z'],
        'Offset X': slider_defaults['Offset X'],
        'Offset Y': slider_defaults['Offset Y'],
        'Offset Z': slider_defaults['Offset Z'],
        'Mode': slider_defaults['Mode']
    }
    ...
    button_height = 40
    margin = 10
    start_x = width - (button_width + margin) * 6  # Увеличиваем на 1 для 6 кнопок
    ...
    # Функция для обновления current_slider_values
    def update_slider_values(win_name):  # параметр называется win_name
        current_slider_values['Scale'] = cv2.getTrackbarPos('Scale', win_name)  # используем win_name
        current_slider_values['Rot X'] = cv2.getTrackbarPos('Rot X', win_name)
        current_slider_values['Rot Y'] = cv2.getTrackbarPos('Rot Y', win_name)
        current_slider_values['Rot Z'] = cv2.getTrackbarPos('Rot Z', win_name)
        current_slider_values['Offset X'] = cv2.getTrackbarPos('Offset X', win_name)
        current_slider_values['Offset Y'] = cv2.getTrackbarPos('Offset Y', win_name)
        current_slider_values['Offset Z'] = cv2.getTrackbarPos('Offset Z', win_name)
        current_slider_values['Mode'] = cv2.getTrackbarPos('Mode: 0pts/1wire/2face', win_name)

    # Обработчик мыши
    def mouse_callback(event, x, y, flags, param):
        nonlocal fullscreen_mode  
        """Обработчик событий мыши"""
        # Извлекаем данные из param
        scale_x = param['scale_x']
        scale_y = param['scale_y']
        offset_x = param['offset_x']  # Смещение по X при центрировании
        offset_y = param['offset_y']  # Смещение по Y при центрировании
        buttons = param['buttons']
        window_name = param['window_name']
        
        # Преобразуем координаты из окна в оригинальный кадр с учетом центрирования
        # Сначала вычитаем смещение, потом делим на масштаб
        ...
        if event == cv2.EVENT_LBUTTONDOWN:
            for button in buttons:
                if button.is_inside(orig_x, orig_y):
                    ...
                    # В mouse_callback добавить обработку новой кнопки
                    elif button.text == "FULL":
                        fullscreen_mode = not fullscreen_mode
                        if fullscreen_mode:
                            print("\n🔍 Полноэкранный режим (слайдеры скрыты)")
                            # Переключаем в полноэкранный режим
                            cv2.setWindowProperty(window_name, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
                            
                            # Пытаемся скрыть слайдеры (устанавливаем их позицию за пределами окна)
                            # Это хак, который работает в некоторых версиях OpenCV
                            try:
                                # Получаем размер окна
                                rect = cv2.getWindowImageRect(window_name)
                                # Перемещаем слайдеры за пределы видимой области
                                for name in ['Scale', 'Rot X', 'Rot Y', 'Rot Z', 'Offset X', 'Offset Y', 'Offset Z', 'Mode: 0pts/1wire/2face']:
                                    cv2.setTrackbarPos(name, window_name, 0)  # Не помогает скрыть, только сбросить
                            except:
                                pass
                        else:
                            print("\n🔍 Обычный режим (слайдеры видны)")
    ...
    try:
        while True:
            ...
            update_slider_values(window_name)
            ...
            # Обновляем коэффициенты масштабирования для mouse callback
