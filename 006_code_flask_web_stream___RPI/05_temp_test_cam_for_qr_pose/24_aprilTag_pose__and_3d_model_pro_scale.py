...
def slider_values_to_params(slider_values):
    ...
    if 'Scale' in slider_values:
        ...
        params['scale'] = slider_values['Scale'] / 100.0
...
if SHOW_SLIDERS:
    ...
    # Scale: (0-100)
    # Получаем значение из конфига или используем 30 по умолчанию
    default_scale = default_values.get('Scale', 30)  # 

    # Создание слайдера Scale
    cv2.createTrackbar(
        'Scale (1.0-2.0)',  # название слайдера
        window_name,         # название окна
        30,                  # начальное значение (по умолчанию 30)
        100,                 # МАКСИМАЛЬНОЕ ЗНАЧЕНИЕ (диапазон от 0 до 100)
        nothing              # функция-обработчик (пустая)
    )
