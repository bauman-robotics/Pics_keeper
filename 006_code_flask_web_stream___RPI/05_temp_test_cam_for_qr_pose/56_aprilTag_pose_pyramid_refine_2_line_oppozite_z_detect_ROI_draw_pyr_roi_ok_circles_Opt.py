...
def init_camera():
    ...
    if CAMERA_TYPE == 'usb':
        # ТОЛЬКО ОДИН РАЗ открываем камеру
        cap = cv2.VideoCapture(16)  # просто индекс, без CAP_V4L2
...
def main():
    ...
    # ===== ОСНОВНОЙ ЦИКЛ =====
    try:
        # ИНИЦИАЛИЗАЦИЯ ПЕРЕМЕННЫХ ДЛЯ ЗАМЕРА ВРЕМЕНИ
        last_time = time.time()
        last_capture_time = time.time()
        frame_count = 0
        ...
        while True:
            ...
            # Проверяем режим VIDEO ONLY
            video_only_mode = btn_video_only is not None and btn_video_only.active

            # ПОЛУЧАЕМ ЗНАЧЕНИЯ СЛАЙДЕРОВ (ВСЕГДА, для окна управления)
            slider_vals = _get_all_sliders()
            params = slider_to_params(slider_vals)
            ...
            # Статус REFINE
            refine_active_local = not video_only_mode and refine_active if 'refine_active' in locals() else False
            refine_color = (0,220,255) if refine_active_local else (120,120,120)
            cv2.putText(controls_frame,
                        f"Line cross refine: {'ACTIVE' if refine_active_local else 'OFF'}",
                        (20, 480), cv2.FONT_HERSHEY_SIMPLEX, 0.5, refine_color, 1)
            
            if refine_active_local:
                cv2.putText(controls_frame,
