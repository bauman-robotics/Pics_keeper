...
def detect_pyramid_corners(frame_gray, rvec, tvec, camera_matrix, dist_coeffs):
    ...
    С отображением всех линий и найденных точек.
    """
    ...
    white_border = 0.002  # 2 мм
    top_size = marker_size + 2 * white_border  # 22 мм
    
    half_top = top_size / 2.0  # 11 мм
    ...
    if DEBUG_PYRAMID:
        ...
        print(f"\n{'='*60}")
        print(f"🔍 PYRAMID DETECTION WITH LINE VISUALIZATION")
        print(f"{'='*60}")
        print(f"📏 Distance: {distance_m:.3f}m")
        print(f"📏 Top face: 22mm (18mm marker + 2mm border x2)")
    
    # Центры граней НА БЕЛОМ ОБОДКЕ (10 мм от центра)
    face_centers_3d = {
        'FRONT': np.array([0.0, -0.010, h_pyr/2]),   # 10 мм по Y
        'BACK': np.array([0.0, 0.010, h_pyr/2]),     # 10 мм по Y
        'LEFT': np.array([-0.010, 0.0, h_pyr/2]),    # 10 мм по X
        'RIGHT': np.array([0.010, 0.0, h_pyr/2])     # 10 мм по X
    }
    ...
    # Пустой результат для повторного использования
    empty_result = {
        'face': '',
        'center_3d': np.array([0,0,0]),
        'center_2d': None,
        'proj_center': np.array([0,0]),
        'reproj_err': None,
        'visible': False,
        'confidence': 0.0
    }
    ...
    for face_idx, face in enumerate(PYRAMID_FACES):
        ...
        # Проецируем нормаль для ориентации ROI
        normal_point_3d = (face_centers_3d[name] + face_normals[name] * 0.005).reshape(1, 1, 3)
        ...
        if DEBUG_PYRAMID:
            ...
            print(f"   📐 Normal: ({normal_dir[0]:.3f}, {normal_dir[1]:.3f})")
            cv2.circle(debug_frame, (int(center_proj[0]), int(center_proj[1])), 
                      6, (0, 255, 255), -1)
        
        if not visible:
            res = empty_result.copy()
            res['face'] = name
            res['center_3d'] = face_centers_3d[name] * 1000.0
            res['proj_center'] = center_proj
            res['visible'] = visible
            results.append(res)
            continue
        ...
        for s1 in [-1, 1]:
            for s2 in [-1, 1]:
                corner = center_proj + s1 * normal_dir * roi_normal/2 + \
                                      s2 * tangent_dir * roi_tangent/2
        ...
        if DEBUG_PYRAMID and lines is not None:
            # Рисуем ВСЕ найденные линии на отладочном кадре
            for line in lines:
                x1_l, y1_l, x2_l, y2_l = line[0]  # line[0] дает кортеж из 4 чисел
                cv2.line(debug_frame, 
                        (x1_l + x1, y1_l + y1),
                        (x2_l + x1, y2_l + y1),
                        (255, 0, 0), 1)  # Синие линии
        
        if lines is None or len(lines) < 3:
            if DEBUG_PYRAMID:
                print(f"   ❌ Not enough lines: {0 if lines is None else len(lines)}")
            res = empty_result.copy()
            res['face'] = name
            res['center_3d'] = face_centers_3d[name] * 1000.0
            res['proj_center'] = center_proj
        ...
        # Классифицируем линии по направлению
        horizontal_lines = []
        vertical_lines = []
        ...
        for line in lines:
            ...
            # Сравниваем с ожидаемыми направлениями
            h_sim = abs(np.dot(dir_vec, tangent_dir))   # вдоль грани
            v_sim = abs(np.dot(dir_vec, normal_dir))    # поперек грани
            
            if h_sim > 0.7:
                horizontal_lines.append(line)
                if DEBUG_PYRAMID:
                    # Рисуем горизонтальные линии зеленым
                    cv2.line(debug_frame,
                            (x1_l + x1, y1_l + y1),
                            (x2_l + x1, y2_l + y1),
                            (0, 255, 0), 2)
                ...
                if DEBUG_PYRAMID:
                    # Рисуем вертикальные линии красным
                    cv2.line(debug_frame,
                            (x1_l + x1, y1_l + y1),
                            (x2_l + x1, y2_l + y1),
                            (0, 0, 255), 2)
        ...
        for h_line in horizontal_lines:
            for v_line in vertical_lines:
                ...
                # Оцениваем качество (ближе к центру ROI - лучше)
        ...
        if DEBUG_PYRAMID:
            # Рисуем все найденные пересечения желтым
            for pt in intersections:
                cv2.circle(debug_frame, (int(pt[0]), int(pt[1])), 4, (0, 255, 255), -1)
        ...
        if DEBUG_PYRAMID:
            # Рисуем лучший центр большим зеленым кругом
