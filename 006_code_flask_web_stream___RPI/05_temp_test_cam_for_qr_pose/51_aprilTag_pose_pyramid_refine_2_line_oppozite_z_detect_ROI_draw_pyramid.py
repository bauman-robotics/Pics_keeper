...
# === ROI ====================================================================
# ============================================================================
def compute_face_roi(frame_shape, name, marker_corners_2d, face_center_proj, 
                     base_size=70, normal_scale=1.8, tangent_scale=1.0):
    """
    Вычисляет ROI вокруг проекции центра грани.
    
    Параметры:
        frame_shape - (h, w) размер кадра
        name - название грани
        marker_corners_2d - углы маркера (не используются, но оставлены для совместимости)
        face_center_proj - проекция центра грани (x, y)
        base_size - базовый размер ROI
        normal_scale - масштаб по вертикали
        tangent_scale - масштаб по горизонтали
    
    Возвращает:
        (x1, y1, x2, y2) - координаты ROI
        (roi_w, roi_h) - размеры ROI
    """
    
    h, w = frame_shape
    x, y = face_center_proj
    
    # Вычисляем размеры ROI
    roi_w = int(base_size * tangent_scale)
    roi_h = int(base_size * normal_scale)
    
    # ROI центрировано на проекции центра грани
    x1 = max(0, int(x - roi_w/2))
    y1 = max(0, int(y - roi_h/2))
    x2 = min(w, int(x + roi_w/2))
    y2 = min(h, int(y + roi_h/2))
    
    if DEBUG_PYRAMID:
        print(f"   📍 Центр грани: ({x:.1f}, {y:.1f})")
        print(f"   📦 ROI: {roi_w}x{roi_h}px вокруг центра")
        print(f"   📍 ROI от ({x1},{y1}) до ({x2},{y2})")
    ...
    info = {
        ...
        'roi_actual': (x2-x1, y2-y1)
    }
    
    return (x1, y1, x2, y2), (roi_w, roi_h), info

def draw_face_roi(debug_frame, roi_coords, info, marker_corners_2d, color=None):
    """
    Рисует ROI на отладочном кадре.
    """
    
    x1, y1, x2, y2 = roi_coords
    name = info['name']
    start_point = info['start_point']
    normal_dir = info['normal_dir']
    tangent_dir = info['tangent_dir']
    ...
    # Рисуем маркер (полупрозрачный)
    overlay = debug_frame.copy()
    cv2.fillPoly(overlay, [marker_corners_2d.astype(np.int32)], (0, 255, 0))
    cv2.addWeighted(overlay, 0.2, debug_frame, 0.8, 0, debug_frame)
    
    # Рисуем стартовую точку на границе
    cv2.circle(debug_frame, (int(start_point[0]), int(start_point[1])), 6, color, -1)
    
    # Рисуем ROI (только наружу)
    ...
    cv2.polylines(debug_frame, [roi_corners.astype(np.int32)], True, color, 2)
    
    # Рисуем направление нормали (красная стрелка наружу)
    cv2.arrowedLine(debug_frame, 
                   (int(start_point[0]), int(start_point[1])),
                   (int(start_point[0] + normal_dir[0]*50), 
                    int(start_point[1] + normal_dir[1]*50)),
                   (0, 0, 255), 2)
    
    # Рисуем касательное направление (синяя стрелка вдоль грани)
    cv2.arrowedLine(debug_frame,
                   (int(start_point[0]), int(start_point[1])),
                   (int(start_point[0] + tangent_dir[0]*40),
                    int(start_point[1] + tangent_dir[1]*40)),
                   (255, 0, 0), 2)
    
    # Подпись
    cv2.putText(debug_frame, name, 
               (int(start_point[0])+20, int(start_point[1])-20),
               cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
    
    return debug_frame

def draw_face_roi(debug_frame, roi_coords, info, color=None):
    """
    Рисует ROI на отладочном кадре.
    """
    ...
    # Рисуем центр грани
    cv2.circle(debug_frame, (int(center[0]), int(center[1])), 8, color, -1)
    cv2.putText(debug_frame, name, 
    ...
    cv2.arrowedLine(debug_frame, 
                   ...
                   (int(center[0] + normal_dir[0]*50), 
                    int(center[1] + normal_dir[1]*50)),
                   (0, 0, 255), 2)  # Красный - нормаль наружу
    
    cv2.arrowedLine(debug_frame,
                   (int(center[0]), int(center[1])),
                   (int(center[0] + tangent_dir[0]*40),
                    int(center[1] + tangent_dir[1]*40)),
                   (255, 0, 0), 2)  # Синий - касательная


def print_roi_info(info, marker_center=None):
    """
    Выводит информацию о ROI в консоль.
    """
    
    name = info['name']
    center = info['center']
    normal_dir = info['normal_dir']
    roi_actual = info['roi_actual_size']
    roi_normal_size = info['roi_normal_size']
    roi_tangent_size = info['roi_tangent_size']
    
    print(f"\n{name}:")
    print(f"   📍 Center proj: ({center[0]:.1f}, {center[1]:.1f})")
    
    if marker_center is not None:
        dist = np.linalg.norm(center - marker_center)
        offset = center - marker_center
        print(f"   📏 От центра маркера: {dist:.1f}px")
        print(f"      Смещение: ({offset[0]:.1f}, {offset[1]:.1f})px")
    
    print(f"   📐 Normal direction: ({normal_dir[0]:.3f}, {normal_dir[1]:.3f})")
    print(f"   📦 ROI: {roi_actual[0]}x{roi_actual[1]}px")
    print(f"      Размер по нормали: {roi_normal_size}px")
    print(f"      Размер по касательной: {roi_tangent_size}px")


def log_face_coordinates(frame_gray, rvec, tvec, camera_matrix, dist_coeffs):
    """
    Вычисляет и выводит в лог координаты центров граней пирамиды.
    """
    ...
    # Центры граней в 3D (на боковых гранях, на половине высоты)
    face_centers_3d = {
        'FRONT': np.array([0.0, -half_top, -h_pyr/2]),   # передняя грань
        'BACK': np.array([0.0, half_top, -h_pyr/2]),     # задняя грань
        'LEFT': np.array([-half_top, 0.0, -h_pyr/2]),    # левая грань
        'RIGHT': np.array([half_top, 0.0, -h_pyr/2])     # правая грань
    }
    
    print(f"\n{'='*60}")
    print(f"📐 КООРДИНАТЫ ЦЕНТРОВ ГРАНЕЙ:")
    print(f"{'='*60}")
    
    # Проецируем углы маркера для справки
    ...
    print(f"\n📍 ЦЕНТР МАРКЕРА: ({marker_center[0]:.1f}, {marker_center[1]:.1f})")
    
    for name, center_3d in face_centers_3d.items():
        # Проецируем центр грани
        center_3d_reshaped = center_3d.reshape(1, 1, 3).astype(np.float32)
        center_2d, _ = cv2.projectPoints(
            center_3d_reshaped, rvec, tvec, camera_matrix, dist_coeffs
        )
        center_2d = center_2d[0][0]
        
        # Расстояние от центра маркера
        dist = np.linalg.norm(center_2d - marker_center)
        
        # Смещение относительно центра маркера
        offset = center_2d - marker_center
        
        print(f"\n🔷 {name}:")
        print(f"   3D координаты: X={center_3d[0]*1000:+.1f}мм, Y={center_3d[1]*1000:+.1f}мм, Z={center_3d[2]*1000:+.1f}мм")
        print(f"   2D проекция: ({center_2d[0]:.1f}, {center_2d[1]:.1f})")
        print(f"   Расстояние от центра маркера: {dist:.1f}px")
        print(f"   Смещение: X={offset[0]:+.1f}px, Y={offset[1]:+.1f}px")
        
        # Проверка, внутри или снаружи маркера
        # Определяем границы маркера
        min_x = np.min(marker_corners_2d[:, 0])
        max_x = np.max(marker_corners_2d[:, 0])
        min_y = np.min(marker_corners_2d[:, 1])
        max_y = np.max(marker_corners_2d[:, 1])
        
        if (min_x <= center_2d[0] <= max_x and min_y <= center_2d[1] <= max_y):
            print(f"   ⚠️ ВНУТРИ маркера!")
        else:
            print(f"   ✅ Снаружи маркера")
...
def detect_pyramid_corners(frame_gray, rvec, tvec, camera_matrix, dist_coeffs):
    ...
    С улучшенной отладкой видимости.
    """
    ...
    if DEBUG_PYRAMID:
        ...
        print(f"\n{'='*60}")
        print(f"🔍 PYRAMID DETECTION (FIXED VISIBILITY)")
        print(f"{'='*60}")
    ...
    if DEBUG_PYRAMID:
        ...
        print(f"   Ширина: {np.linalg.norm(marker_corners_2d[1] - marker_corners_2d[0]):.1f}px")
        print(f"   Высота: {np.linalg.norm(marker_corners_2d[2] - marker_corners_2d[1]):.1f}px")
        ...
        cv2.putText(debug_frame, "MARKER", 
                   (int(marker_corners_2d[0][0]), int(marker_corners_2d[0][1])-10),
                   cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
        cv2.circle(debug_frame, (int(marker_center[0]), int(marker_center[1])), 5, (255, 0, 0), -1)
    

        # ========== ВОТ ЗДЕСЬ ДОБАВЛЯЕМ РЕБРА ПИРАМИДЫ ==========
        debug_frame = draw_pyramid_edges(debug_frame, rvec, tvec, camera_matrix, dist_coeffs)
        # =========================================================

        # Логируем координаты центров граней
        log_face_coordinates(frame_gray, rvec, tvec, camera_matrix, dist_coeffs)
    ...
    for face_idx, face in enumerate(PYRAMID_FACES):
        ...
        if DEBUG_PYRAMID:
            print(f"   📊 dot продукт: {dot_val:.3f}")
            print(f"   👁️ Видимость: {visible}")
        
        if not visible:
            if DEBUG_PYRAMID:
                print(f"   ⏭️ Грань НЕ ВИДНА - пропускаем")
        ...
        if DEBUG_PYRAMID:
            print(f"   ✅ Грань ВИДНА - обрабатываем")
        ...
        # Проецируем точку вдоль нормали
        normal_scale_3d = 0.005
        normal_point_3d = (face_centers_3d[name] + face_normals[name] * normal_scale_3d).reshape(1, 1, 3)
        normal_point_2d, _ = cv2.projectPoints(
            normal_point_3d, rvec, tvec, camera_matrix, dist_coeffs
        )
        normal_point_2d = normal_point_2d[0][0]
        
        # Направление нормали
        normal_dir = normal_point_2d - center_proj
        ...
        # ВЫЧИСЛЯЕМ ROI
        roi_coords, roi_sizes, roi_info = compute_face_roi(
            frame_gray.shape, name, marker_corners_2d, center_proj
        )
        x1, y1, x2, y2 = roi_coords
        
        if DEBUG_PYRAMID:
            print(f"   📍 Center proj: ({center_proj[0]:.1f}, {center_proj[1]:.1f})")
            dist_from_center = np.linalg.norm(center_proj - marker_center)
            print(f"   📏 От центра маркера: {dist_from_center:.1f}px")
            print(f"   📐 Normal direction: ({normal_dir[0]:.3f}, {normal_dir[1]:.3f})")
            ...
            color = colors.get(name, (255, 255, 255))
            cv2.circle(debug_frame, (int(center_proj[0]), int(center_proj[1])), 6, color, -1)
            
            # Рисуем ROI
            cv2.rectangle(debug_frame, (x1, y1), (x2, y2), color, 2)
            cv2.putText(debug_frame, f"{name} (visible)", (x1, y1-5), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
        ...
        if roi.size == 0 or roi.shape[0] < 30 or roi.shape[1] < 30:
            if DEBUG_PYRAMID:
                print(f"   ⚠️ ROI слишком маленький")
            results.append({
                'face': name,
                'center_3d': face_centers_3d[name] * 1000.0,
                'center_2d': None,
                'proj_center': center_proj,
                'reproj_err': None,
                'visible': visible,
                'confidence': 0.0
            })
        ...
        # ========== ПОИСК ЛИНИЙ ==========
        if DEBUG_PYRAMID:
            print(f"   🔍 Поиск линий в ROI {roi.shape}")
        ...
        for line in lines:
            ...
            h_sim = abs(np.dot(dir_vec, tangent_dir))
            v_sim = abs(np.dot(dir_vec, normal_dir))
            
            # if h_sim > 0.7:
            #     horizontal_lines.append(line)
            #     if DEBUG_PYRAMID:
            #         print(f"      Горизонтальная линия: sim={h_sim:.2f}")
            # elif v_sim > 0.7:
            #     vertical_lines.append(line)
            #     if DEBUG_PYRAMID:
            #         print(f"      Вертикальная линия: sim={v_sim:.2f}")
        
        if DEBUG_PYRAMID:
            print(f"   📊 Горизонтальных: {len(horizontal_lines)}, Вертикальных: {len(vertical_lines)}")
    ...
    if DEBUG_PYRAMID:
        ...
        for r in results:
            status = "FOUND" if r['center_2d'] is not None else "NOT FOUND"
            vis = "VISIBLE" if r['visible'] else "HIDDEN"
            if r['center_2d'] is not None:
                print(f"   {r['face']}: {status}, {vis}, error {r['reproj_err']:.1f}px, conf {r['confidence']:.2f}")
            else:
                print(f"   {r['face']}: {status}, {vis}")
        print(f"{'='*60}")
