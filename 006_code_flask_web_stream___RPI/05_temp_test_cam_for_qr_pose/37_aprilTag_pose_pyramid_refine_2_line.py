...
def detect_pyramid_corners(frame_gray, rvec, tvec, camera_matrix, dist_coeffs):
    ...
    # Размер ROI (используем размер грани)
    face_size_mm = (PYRAMID_BASE_SIZE + PYRAMID_TOP_SIZE) / 2.0  # средний размер грани
    ...
    for face in PYRAMID_FACES:
        ...
        # --- Поиск точки пересечения вертикальной и горизонтальной линий ---
        # Используем комбинацию детектора линий и поиска пересечения

        # 1. Улучшение контраста и бинаризация
        roi_eq = cv2.equalizeHist(roi)
        _, roi_binary = cv2.threshold(roi_eq, 127, 255, cv2.THRESH_BINARY_INV)

        # 2. Выделение линий с помощью морфологических операций
        # Горизонтальная линия
        horizontal_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (25, 1))
        horizontal_lines = cv2.morphologyEx(roi_binary, cv2.MORPH_OPEN, horizontal_kernel)

        # Вертикальная линия
        vertical_kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (1, 25))
        vertical_lines = cv2.morphologyEx(roi_binary, cv2.MORPH_OPEN, vertical_kernel)

        # 3. Комбинируем линии
        lines_combined = cv2.bitwise_and(horizontal_lines, vertical_lines)

        # 4. Поиск точки пересечения (центр перекрестия)
        # Находим все белые пиксели (пересечения)
        y_coords, x_coords = np.where(lines_combined > 0)

        if len(x_coords) < 5:  # Недостаточно пикселей для надежного определения
            if DEBUG_PYRAMID:
                print(f"  Face {name}: no intersection point found in ROI")
            results.append(result_base)
            continue

        # Вычисляем центр масс всех точек пересечения
        cx_roi = float(np.mean(x_coords))
        cy_roi = float(np.mean(y_coords))

        # 5. Субпиксельное уточнение вокруг найденной точки
