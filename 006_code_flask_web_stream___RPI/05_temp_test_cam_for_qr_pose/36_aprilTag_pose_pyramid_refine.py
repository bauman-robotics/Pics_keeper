...
def detect_pyramid_corners(frame_gray, rvec, tvec, camera_matrix, dist_coeffs):
    ...
    for face in PYRAMID_FACES:
        ...
        # --- Поиск угловой точки шахматного уголка 2x2 в ROI ---
        # findChessboardCorners требует patternSize >= (2,2) внутренних точек,
        # что соответствует доске 3x3 квадрата — слишком много для нашего паттерна.
        # Для паттерна 2x2 квадрата (1 точка пересечения) используем
        # goodFeaturesToTrack (детектор Харриса) + cornerSubPix.

        # Предобработка ROI для улучшения контраста =============================================
        # roi_eq = cv2.equalizeHist(roi)

        # # Детектируем угловые точки Харриса в ROI
        # corners_gftt = cv2.goodFeaturesToTrack(
        #     roi_eq,
        #     maxCorners=5,           # ищем не более 5 кандидатов
        #     qualityLevel=0.01,      # низкий порог — не пропустить слабые углы
        ...
        # if corners_gftt is None or len(corners_gftt) == 0:
        #     if DEBUG_PYRAMID:
        #         print(f"  Face {name}: corner NOT FOUND in ROI "
        #               f"({x1},{y1},{x2-x1},{y2-y1}), proj=({px:.0f},{py:.0f})")
        #     results.append(result_base)
        #     continue

        # # Выбираем точку, ближайшую к центру ROI
        # # (центр ROI соответствует проекции центра грани)
        # roi_cx = (x2 - x1) / 2.0
        # roi_cy = (y2 - y1) / 2.0
        # best_corner = min(
        #     corners_gftt,
        #     key=lambda c: (c[0][0] - roi_cx)**2 + (c[0][1] - roi_cy)**2
        # )
        # best_corner = best_corner.reshape(1, 1, 2).astype(np.float32)
        ...
        blurred = cv2.GaussianBlur(roi_float, (5, 5), 1.5)

        # Матрица Хессе — ищем седловую точку
        ...
        # Ищем в центральной зоне ROI (50% от размера)
        margin = max(roi.shape[0] // 4, 5)
        det_center = det_H[margin:-margin, margin:-margin]

        if det_center.size == 0 or np.min(det_center) >= 0:
            if DEBUG_PYRAMID:
                print(f"  Face {name}: no saddle point in ROI")
            results.append(result_base)
            continue

        min_loc = np.unravel_index(np.argmin(det_center), det_center.shape)
        cx_roi = float(min_loc[1] + margin)
        cy_roi = float(min_loc[0] + margin)

        # Субпиксельное уточнение вокруг найденной точки
