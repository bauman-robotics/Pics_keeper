...
def detect_pyramid_corners(frame_gray, rvec, tvec, camera_matrix, dist_coeffs):
    ...
    Линии ищутся в направлениях, соответствующих осям грани в 3D.
    """
    results = []
    ...
    # Матрица поворота из вектора поворота
    R, _ = cv2.Rodrigues(rvec)

    face_size_mm = (PYRAMID_BASE_SIZE + PYRAMID_TOP_SIZE) / 2.0
    roi_size = compute_roi_size(face_size_mm, distance_m, fx)
    roi_size = int(roi_size * 1.5)
    ...
    for face in PYRAMID_FACES:
        ...
        # ========== НОВЫЙ ПОДХОД: ПОИСК ЛИНИЙ В 3D ==========
        
        # 1. Определяем направления линий в 3D для этой грани
        # Для каждой грани lines_3d - это два перпендикулярных направления
        # в плоскости грани (в системе координат маркера)
        
        if name == 'FRONT' or name == 'BACK':
            # Для FRONT/BACK граней: оси X и Z в плоскости грани
            line1_dir_3d = np.array([1.0, 0.0, 0.0])  # Вдоль X
            line2_dir_3d = np.array([0.0, 0.0, 1.0])  # Вдоль Z (вниз)
        else:  # LEFT или RIGHT
            # Для LEFT/RIGHT граней: оси Y и Z в плоскости грани
            line1_dir_3d = np.array([0.0, 1.0, 0.0])  # Вдоль Y
            line2_dir_3d = np.array([0.0, 0.0, 1.0])  # Вдоль Z (вниз)
        
        # 2. Трансформируем направления в систему координат камеры
        line1_dir_cam = R @ line1_dir_3d
        line2_dir_cam = R @ line2_dir_3d
        
        # 3. Проецируем направления на изображение
        # Берем точку центра и точки в направлении линий
        p1_3d = center_3d / 1000.0
        p1_line1_3d = p1_3d + line1_dir_3d * 0.01  # 10 мм в направлении линии 1
        p1_line2_3d = p1_3d + line2_dir_3d * 0.01  # 10 мм в направлении линии 2
        
        # Проецируем в 2D
        p1_proj, _ = cv2.projectPoints(
            np.array([p1_3d], dtype=np.float32), rvec, tvec, camera_matrix, dist_coeffs
        )
        p1_line1_proj, _ = cv2.projectPoints(
            np.array([p1_line1_3d], dtype=np.float32), rvec, tvec, camera_matrix, dist_coeffs
        )
        p1_line2_proj, _ = cv2.projectPoints(
            np.array([p1_line2_3d], dtype=np.float32), rvec, tvec, camera_matrix, dist_coeffs
        )
        
        # Вычисляем направления в пикселях
        p1_2d = p1_proj[0][0]
        line1_dir_2d = p1_line1_proj[0][0] - p1_2d
        line2_dir_2d = p1_line2_proj[0][0] - p1_2d
        
        # Нормализуем
        line1_dir_2d = line1_dir_2d / np.linalg.norm(line1_dir_2d)
        line2_dir_2d = line2_dir_2d / np.linalg.norm(line2_dir_2d)
        
        # 4. Теперь ищем линии в ROI, используя эти направления
        ...
        # 5. Создаем ориентированные ядра для поиска линий
        roi_h, roi_w = roi_binary.shape
        kernel_length = max(roi_w // 5, 10)
        
        # Создаем ядра в нужных направлениях
        def create_line_kernel(direction, length):
            """Создает ядро для поиска линии в заданном направлении"""
        ...
        # Создаем ядра для двух направлений
        kernel1 = create_line_kernel(line1_dir_2d, kernel_length)
        kernel2 = create_line_kernel(line2_dir_2d, kernel_length)
        
        # Применяем морфологическое открытие с ориентированными ядрами
        lines1 = cv2.morphologyEx(roi_binary, cv2.MORPH_OPEN, kernel1)
        lines2 = cv2.morphologyEx(roi_binary, cv2.MORPH_OPEN, kernel2)
        
        # 6. Ищем пересечения
        # Для каждой точки проверяем, принадлежит ли она обеим линиям
        intersection = cv2.bitwise_and(lines1, lines2)
        
        # Находим координаты пересечений
        y_coords, x_coords = np.where(intersection > 0)
        
        if len(x_coords) < 5:
            if DEBUG_PYRAMID:
                print(f"  Face {name}: not enough intersection points ({len(x_coords)})")
            results.append(result_base)
            continue
        
        # Центр масс
        cx_roi = float(np.mean(x_coords))
        cy_roi = float(np.mean(y_coords))
        
        # 7. Проверка качества
        # Точки должны лежать примерно на линии в обоих направлениях
        x_std = np.std(x_coords)
        y_std = np.std(y_coords)
        
        # Проецируем точки на направления линий для проверки
        points = np.column_stack((x_coords, y_coords))
        center_point = np.array([cx_roi, cy_roi])
        
        # Проекции на направления
        proj1 = np.dot(points - center_point, line1_dir_2d)
        proj2 = np.dot(points - center_point, line2_dir_2d)
        
        # Дисперсия вдоль и поперек линий
        var_along1 = np.var(proj1)
        var_across1 = np.var(proj2)  # перпендикулярно линии 1
        
        # Если точки слишком разбросаны поперек линии - низкое качество
        if var_across1 > 5.0:
            if DEBUG_PYRAMID:
                print(f"  Face {name}: points too scattered across line (var={var_across1:.1f})")
            results.append(result_base)
            continue
        
        # 8. Субпиксельное уточнение
        ...
        # 9. Репроекционная ошибка
        reproj_err = float(np.linalg.norm(corner_full - np.array([px, py])))
        
        if reproj_err > PYRAMID_REPROJ_THRESHOLD:
            if DEBUG_PYRAMID:
                print(f"  Face {name}: reproj_err={reproj_err:.1f}px > threshold")
            results.append(result_base)
            continue
        
        # Уверенность
        confidence = min(1.0, (len(x_coords) / 30.0) * (1.0 / (1.0 + var_across1)))
