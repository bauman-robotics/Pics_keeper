...
def detect_pyramid_corners(frame_gray, rvec, tvec, camera_matrix, dist_coeffs):
    ...
    С адаптивным ROI, учитывающим угол обзора.
    """
    results = []
    ...
    for face in PYRAMID_FACES:
        ...
        # 1. Проецируем все 4 угла грани, чтобы найти реальные границы
        # Создаем 4 угла грани в 3D
        ...
        if (x2 - x1) < 40 or (y2 - y1) < 40:
            if DEBUG_PYRAMID:
                print(f"  Face {name}: ROI too small ({x2-x1}x{y2-y1})")
            results.append(result_base)
            continue
        ...
        if DEBUG_PYRAMID:
            print(f"  Face {name}: ROI size={x2-x1}x{y2-y1}, "
                  f"face_size={face_width:.0f}x{face_height:.0f}px")
        ...
        # Определяем направления линий в 3D
        if name == 'FRONT' or name == 'BACK':
            line1_dir_3d = np.array([1.0, 0.0, 0.0])  # Вдоль X
            line2_dir_3d = np.array([0.0, 0.0, 1.0])  # Вдоль Z
        else:  # LEFT или RIGHT
            line1_dir_3d = np.array([0.0, 1.0, 0.0])  # Вдоль Y
            line2_dir_3d = np.array([0.0, 0.0, 1.0])  # Вдоль Z
        
        # Трансформируем в камеру
        line1_dir_cam = R @ line1_dir_3d
        line2_dir_cam = R @ line2_dir_3d
        
        # Проецируем точки для определения направлений в 2D
        p1_3d = center_3d / 1000.0
        p1_line1_3d = p1_3d + line1_dir_3d * 0.01
        p1_line2_3d = p1_3d + line2_dir_3d * 0.01
        ...
        # Направления в пикселях (в системе координат ROI)
        p1_2d = p1_proj[0][0] - np.array([x1, y1])
        line1_dir_2d = (p1_line1_proj[0][0] - np.array([x1, y1])) - p1_2d
        line2_dir_2d = (p1_line2_proj[0][0] - np.array([x1, y1])) - p1_2d
        
        # Нормализуем
        line1_dir_2d = line1_dir_2d / (np.linalg.norm(line1_dir_2d) + 1e-6)
        line2_dir_2d = line2_dir_2d / (np.linalg.norm(line2_dir_2d) + 1e-6)
        
        # Поиск линий с помощью преобразования Хафа
        edges = cv2.Canny(roi_binary, 50, 150, apertureSize=3)
        ...
        # Классифицируем линии по направлениям
        lines_list = lines[:, 0, :]
        line_angles = []
        line_centers = []
        ...
        for line in lines_list:
            ...
            angle = np.arctan2(y2_l - y1_l, x2_l - x1_l)
        ...
        # Находим линии, близкие к ожидаемым направлениям
        angle1_target = np.arctan2(line1_dir_2d[1], line1_dir_2d[0])
        angle2_target = np.arctan2(line2_dir_2d[1], line2_dir_2d[0])
        
        # Учитываем периодичность углов
        angle_diff1 = np.minimum(
            np.abs(line_angles - angle1_target),
            np.abs(line_angles - angle1_target - np.pi)
        )
        angle_diff2 = np.minimum(
            np.abs(line_angles - angle2_target),
            np.abs(line_angles - angle2_target - np.pi)
        )
        
        # Выбираем лучшие линии для каждого направления
        threshold_angle = np.pi / 8  # 22.5 градусов
        
        mask1 = angle_diff1 < threshold_angle
        mask2 = angle_diff2 < threshold_angle
        
        if not np.any(mask1) or not np.any(mask2):
            if DEBUG_PYRAMID:
                print(f"  Face {name}: missing line directions")
            results.append(result_base)
            continue
        
        # Берем линии, ближайшие к центру
        dist_to_center = np.linalg.norm(
            line_centers - np.array([roi.shape[1]/2, roi.shape[0]/2]), axis=1
        )
        
        best_line1_idx = np.argmin(dist_to_center * (~mask1) * 1e6)  # игнорируем неподходящие
        best_line2_idx = np.argmin(dist_to_center * (~mask2) * 1e6)
        
        # Находим пересечение двух линий
        line1 = lines_list[best_line1_idx]
        line2 = lines_list[best_line2_idx]
        
        # Вычисляем пересечение
        x1_l1, y1_l1, x2_l1, y2_l1 = line1
        x1_l2, y1_l2, x2_l2, y2_l2 = line2
        
        # Параметры линий: a*x + b*y + c = 0
        a1 = y2_l1 - y1_l1
        b1 = x1_l1 - x2_l1
        c1 = x2_l1 * y1_l1 - x1_l1 * y2_l1
        
        a2 = y2_l2 - y1_l2
        b2 = x1_l2 - x2_l2
        c2 = x2_l2 * y1_l2 - x1_l2 * y2_l2
        ...
        cx_roi = (b1 * c2 - b2 * c1) / det
        cy_roi = (c1 * a2 - c2 * a1) / det
        
        # Проверяем, что пересечение в пределах ROI
        if cx_roi < 0 or cx_roi >= roi.shape[1] or cy_roi < 0 or cy_roi >= roi.shape[0]:
            if DEBUG_PYRAMID:
                print(f"  Face {name}: intersection outside ROI")
            results.append(result_base)
            continue
