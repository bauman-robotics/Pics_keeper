...
def detect_pyramid_corners(frame_gray, rvec, tvec, camera_matrix, dist_coeffs):
    ...
    Специальная версия для толстых линий (2 мм толщиной).
    ...
    if DEBUG_PYRAMID:
        ...
        print(f"\n{'='*60}")
        print(f"🔍 PYRAMID CORNER DETECTION DEBUG (THICK LINES) - Frame {h}x{w}")
        print(f"{'='*60}")
    
    # Размеры пирамиды в метрах
    half_top = PYRAMID_TOP_SIZE / 2.0 / 1000.0
    half_base = PYRAMID_BASE_SIZE / 2.0 / 1000.0
    h_pyr = PYRAMID_HEIGHT / 1000.0
    
    if DEBUG_PYRAMID:
        print(f"\n📐 LINE THICKNESS: {LINE_WIDTH}mm (физическая)")
        print(f"   Отношение толщины к размеру грани: {LINE_WIDTH/11.0*100:.1f}%")
    ...
    for face_idx, face in enumerate(PYRAMID_FACES):
        ...
        # Проекция центра грани (для справки)
        ...
        if not visible:
            if DEBUG_PYRAMID:
                print(f"   ⏭️ Face not visible - skipping")
        ...
        # Проецируем верхнюю и нижнюю точки
        ...
        if DEBUG_PYRAMID:
            print(f"   📐 Directions:")
            print(f"      Vertical: ({vertical_dir[0]:.3f}, {vertical_dir[1]:.3f})")
            print(f"      Horizontal: ({horizontal_dir[0]:.3f}, {horizontal_dir[1]:.3f})")

        # ========== ПРОЕКЦИЯ УГЛОВ ГРАНИ И ROI ==========
        # Определяем 3D координаты 4 углов грани
        if name == 'FRONT':
        ...
        # Увеличиваем отступ для толстых линий
        ...
        if roi_width < 50 or roi_height < 50:
            if DEBUG_PYRAMID:
                print(f"   ⚠️ ROI too small: {roi_width}x{roi_height}")
            results.append(result_base)
            continue
        ...
        if DEBUG_PYRAMID:
            print(f"   📦 ROI: ({x1}, {y1}) to ({x2}, {y2}) size {roi_width}x{roi_height}")
            cv2.rectangle(debug_frame, (x1, y1), (x2, y2), (255, 255, 0), 1)

        # ========== СПЕЦИАЛЬНЫЙ ПОИСК ДЛЯ ТОЛСТЫХ ЛИНИЙ ==========
        
        # 1. Усиленное размытие для сглаживания краев толстых линий
        roi_blur = cv2.GaussianBlur(roi, (7, 7), 1.5)
        
        # 2. Адаптивная бинализация для выделения линий
        ...
        # 3. Морфологические операции для выделения центра линий
        kernel = np.ones((5, 5), np.uint8)  # Большое ядро для толстых линий
        
        # Открытие (убирает шум)
        binary = cv2.morphologyEx(binary, cv2.MORPH_OPEN, kernel, iterations=1)
        
        # Скелетизация - выделение центральной линии
        # Используем distance transform для нахождения центров
        dist = cv2.distanceTransform(binary, cv2.DIST_L2, 5)
        # Нормализуем для визуализации
        cv2.normalize(dist, dist, 0, 1.0, cv2.NORM_MINMAX)
        
        # Находим локальные максимумы (центры линий)
        local_max = np.zeros_like(binary)
        for y in range(1, roi_height-1):
            for x in range(1, roi_width-1):
                if dist[y, x] > dist[y-1, x] and dist[y, x] > dist[y+1, x] and \
                   dist[y, x] > dist[y, x-1] and dist[y, x] > dist[y, x+1]:
                    local_max[y, x] = 255
        
        # Уточняем скелет через порог
        skeleton = (local_max > 0).astype(np.uint8) * 255
        
        if DEBUG_PYRAMID:
            # Показываем промежуточные результаты
            debug_roi = debug_frame[y1:y2, x1:x2].copy()
            # Рисуем бинарное изображение
            binary_colored = cv2.cvtColor(binary, cv2.COLOR_GRAY2BGR)
            binary_colored = cv2.resize(binary_colored, (200, 200))
            debug_frame[10:210, 10:210] = binary_colored
            cv2.putText(debug_frame, "Binary", (10, 220),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
            
            # Рисуем скелет
            skeleton_colored = cv2.cvtColor(skeleton, cv2.COLOR_GRAY2BGR)
            skeleton_colored = cv2.resize(skeleton_colored, (200, 200))
            debug_frame[10:210, 220:420] = skeleton_colored
            cv2.putText(debug_frame, "Skeleton", (220, 220),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 255, 255), 1)
        
        # 4. Поиск линий на скелете
        min_line_length = max(roi_width // 6, 20)  # Адаптивная длина
        lines = cv2.HoughLinesP(
            skeleton,
            rho=1,
            theta=np.pi/180,
            threshold=15,  # Низкий порог для тонких линий
        ...
        if lines is None or len(lines) < 3:
            if DEBUG_PYRAMID:
                print(f"   ⚠️ Not enough lines found on skeleton: {0 if lines is None else len(lines)}")
            results.append(result_base)
            continue
        ...
        if DEBUG_PYRAMID:
            print(f"   📊 Found {len(lines_list)} lines on skeleton")
        
        # 5. Классификация линий
        ...
        for line in lines_list:
            ...
            # Сходство с направлениями
            h_similarity = abs(np.dot(dir_vec, horizontal_dir))
            v_similarity = abs(np.dot(dir_vec, vertical_dir))
            
            line_info = {
                'coords': (x1_l + x1, y1_l + y1, x2_l + x1, y2_l + y1),  # Глобальные
            ...
            all_lines_debug.append(line_info)
            
            # Классификация с мягкими порогами
            if h_similarity > 0.5:  # Еще ниже порог
        ...
        if DEBUG_PYRAMID:
            print(f"   📊 Classified: {len(horizontal_candidates)} horizontal, {len(vertical_candidates)} vertical")
            # Рисуем все линии
            for line_info in all_lines_debug:
                x1_g, y1_g, x2_g, y2_g = line_info['coords']
                if line_info['h_sim'] > 0.5:
                    color = (0, 0, 255)  # Красный
                elif line_info['v_sim'] > 0.5:
                    color = (0, 255, 0)  # Зеленый
                else:
                    color = (100, 100, 100)  # Серый
                cv2.line(debug_frame, (int(x1_g), int(y1_g)), 
                        (int(x2_g), int(y2_g)), color, 1)

        if len(horizontal_candidates) == 0 or len(vertical_candidates) == 0:
            if DEBUG_PYRAMID:
                print(f"   ⚠️ Missing orientation")
            results.append(result_base)
            continue

        # 6. Поиск лучшего пересечения
        ...
        for h_cand in horizontal_candidates:
            for v_cand in vertical_candidates:
                h_line = h_cand['line']
                v_line = v_cand['line']
                ...
                cx = (b_h * c_v - b_v * c_h) / det
                cy = (c_h * a_v - c_v * a_h) / det
                
                # Проверка границ
                margin = 10
                if (cx < margin or cx >= roi_width - margin or 
                    cy < margin or cy >= roi_height - margin):
                    continue
                
                # Оценка качества пересечения
                dist_to_center = np.sqrt((cx - roi_center_x)**2 + (cy - roi_center_y)**2)
                norm_dist = dist_to_center / (roi_width/2)
                
                # Учитываем длины линий
                length_score = (h_cand['length'] + v_cand['length']) / (roi_width + roi_height)
                
                # Учитываем направления
                dir_score = h_cand['similarity'] * v_cand['similarity']
                
                # Комбинированная оценка
                score = dir_score * 0.4 + length_score * 0.3 + (1.0 - norm_dist) * 0.3
                
                if score > best_score:
                    best_score = score
                    best_intersection = (cx, cy)
                    best_h_line = h_line
                    best_v_line = v_line
        
        if best_intersection is None:
            if DEBUG_PYRAMID:
                print(f"   ⚠️ No valid intersection")
            results.append(result_base)
            continue
        
        cx_roi, cy_roi = best_intersection
        
        if DEBUG_PYRAMID:
            print(f"   ✅ Best intersection at ROI ({cx_roi:.1f}, {cy_roi:.1f})")
            # Рисуем лучшие линии
            if best_h_line is not None:
                x1_h, y1_h, x2_h, y2_h = best_h_line
                cv2.line(debug_frame, 
                        (int(x1_h+x1), int(y1_h+y1)),
                        (int(x2_h+x1), int(y2_h+y1)),
                        (0, 0, 255), 2)
            if best_v_line is not None:
                x1_v, y1_v, x2_v, y2_v = best_v_line
                cv2.line(debug_frame,
                        (int(x1_v+x1), int(y1_v+y1)),
                        (int(x2_v+x1), int(y2_v+y1)),
                        (255, 0, 0), 2)

        # 7. Субпиксельное уточнение
        try:
            candidate = np.array([[[cx_roi, cy_roi]]], dtype=np.float32)
            corners_subpix = cv2.cornerSubPix(
                roi,
                candidate,
                winSize=(15, 15),  # Увеличенное окно для толстых линий
                zeroZone=(-1, -1),
        ...
        # Более мягкий порог для толстых линий
        if reproj_err > PYRAMID_REPROJ_THRESHOLD * (1.5 + distance_m):
            if DEBUG_PYRAMID:
                print(f"   ⚠️ Reprojection error too high: {reproj_err:.1f}px")
            results.append(result_base)
            continue
        
        # Уверенность
        confidence = min(1.0, best_score * (1.0 - reproj_err / (PYRAMID_REPROJ_THRESHOLD * 3)))
        confidence = max(0.3, confidence)
        
        if DEBUG_PYRAMID:
            print(f"   ✅ SUCCESS! Confidence: {confidence:.2f}")
            # Рисуем найденный центр
        ...
        if DEBUG_PYRAMID:
            result['debug_info'] = {
                'roi': (x1, y1, x2, y2),
                'lines_found': len(lines_list),
                'horizontal': len(horizontal_candidates),
                'vertical': len(vertical_candidates),
                'intersection_roi': (cx_roi, cy_roi),
                'all_lines': all_lines_debug,
                'best_horizontal': best_h_line,
                'best_vertical': best_v_line
            }
    ...
    if DEBUG_PYRAMID:
        print(f"\n{'='*60}")
        print(f"📊 SUMMARY: Found {sum(1 for r in results if r['center_2d'] is not None)}/4 centers")
        print(f"{'='*60}")
