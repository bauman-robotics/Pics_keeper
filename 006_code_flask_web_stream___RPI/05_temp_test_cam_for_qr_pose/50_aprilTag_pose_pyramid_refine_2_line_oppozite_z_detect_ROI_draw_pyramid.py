...
# === ROI ====================================================================
# ============================================================================
def compute_face_roi(frame_shape, name, marker_corners_2d, normal_dir, tangent_dir, 
                     base_size=70, normal_scale=1.8, tangent_scale=1.0):
    """
    Вычисляет координаты ROI для грани пирамиды.
    ROI смещен НАРУЖУ от центра маркера.
    """
    
    h, w = frame_shape
    
    # Убеждаемся, что marker_corners_2d имеет правильную форму (4, 2)
    if len(marker_corners_2d.shape) == 3:
        marker_corners_2d = marker_corners_2d.reshape(-1, 2)
    
    # Индексы углов маркера
    # 0 - левый верхний
    # 1 - правый верхний
    # 2 - правый нижний
    # 3 - левый нижний
    ...
    if DEBUG_PYRAMID:
        print(f"   📐 Форма marker_corners: {marker_corners_2d.shape}")
        print(f"   📐 Центр маркера: ({marker_center[0]:.1f}, {marker_center[1]:.1f})")
    
    # Определяем середины ребер
    edges = {
        'TOP': (marker_corners_2d[0] + marker_corners_2d[1]) / 2,
        'RIGHT': (marker_corners_2d[1] + marker_corners_2d[2]) / 2,
        'BOTTOM': (marker_corners_2d[2] + marker_corners_2d[3]) / 2,
        'LEFT': (marker_corners_2d[3] + marker_corners_2d[0]) / 2
    }
    
    # Выбираем нужное ребро в зависимости от грани
    if name == 'FRONT':
        start_point = edges['LEFT']
        if DEBUG_PYRAMID:
            print(f"   🎯 FRONT: левое ребро")
        
    elif name == 'BACK':
        start_point = edges['RIGHT']
        if DEBUG_PYRAMID:
            print(f"   🎯 BACK: правое ребро")
        
    elif name == 'LEFT':
        start_point = edges['TOP']
        if DEBUG_PYRAMID:
            print(f"   🎯 LEFT: верхнее ребро")
        
    elif name == 'RIGHT':
        start_point = edges['BOTTOM']
        if DEBUG_PYRAMID:
            print(f"   🎯 RIGHT: нижнее ребро")
    
    if DEBUG_PYRAMID:
        print(f"   📍 Стартовая точка: ({start_point[0]:.1f}, {start_point[1]:.1f})")
    
    # Вектор от стартовой точки к центру маркера
    to_center = marker_center - start_point
    dist_to_center = np.linalg.norm(to_center)
    
    if dist_to_center > 0:
        to_center = to_center / dist_to_center
    
    # Нормаль должна быть направлена ОТ центра маркера
    # (противоположно вектору to_center)
    outward_dir = -to_center
    
    if DEBUG_PYRAMID:
        print(f"   📐 Вектор к центру: ({to_center[0]:.3f}, {to_center[1]:.3f})")
        print(f"   📐 Наружу: ({outward_dir[0]:.3f}, {outward_dir[1]:.3f})")
        print(f"   📐 Расстояние до центра: {dist_to_center:.1f}px")
    
    # Вычисляем размеры ROI
    roi_normal = int(base_size * normal_scale)
    roi_tangent = int(base_size * tangent_scale)
    
    # ROI начинается от стартовой точки и идет наружу
    # НЕ вниз, а именно ОТ центра маркера!
    corners = []
    
    # Смещаем ROI наружу, чтобы оно не заходило на маркер
    start_point_out = start_point + outward_dir * 10  # Отступ 10px наружу
    
    for s2 in [-1, 1]:  # вдоль грани (обе стороны)
        # Касательное направление - перпендикуляр к outward_dir
        tangent = np.array([-outward_dir[1], outward_dir[0]])
        corner = start_point_out + outward_dir * roi_normal + \
                                 s2 * tangent * roi_tangent/2
        corners.append(corner)
    
    corners.append(start_point_out)
    corners = np.array(corners)
    
    # Ограничиваем изображением
    x1 = max(0, int(np.min(corners[:, 0])))
    y1 = max(0, int(np.min(corners[:, 1])))
    x2 = min(w, int(np.max(corners[:, 0])))
    y2 = min(h, int(np.max(corners[:, 1])))
    
    # Проверка: ROI должно быть дальше от центра, чем стартовая точка
    roi_center = np.array([(x1+x2)/2, (y1+y2)/2])
    dist_roi = np.linalg.norm(roi_center - marker_center)
    
    if DEBUG_PYRAMID:
        print(f"   📊 Расстояние от центра: старт={dist_to_center:.1f}, ROI={dist_roi:.1f}")
        if dist_roi > dist_to_center:
            print(f"   ✅ ROI снаружи маркера")
        else:
            print(f"   ⚠️ ROI внутри маркера!")
        print(f"   📍 ROI от ({x1},{y1}) до ({x2},{y2})")
    
    info = {
        'name': name,
        'start_point': start_point,
        'start_point_out': start_point_out,
        'outward_dir': outward_dir,
        'roi_normal_size': roi_normal,
        'roi_tangent_size': roi_tangent,
        'roi_actual_size': (x2-x1, y2-y1),
...
def detect_pyramid_corners(frame_gray, rvec, tvec, camera_matrix, dist_coeffs):
    ...
    for face_idx, face in enumerate(PYRAMID_FACES):
        ...
        roi_coords, roi_sizes, roi_info = compute_face_roi(
            frame_gray.shape, name, marker_corners_2d, normal_dir, tangent_dir
        )
        x1, y1, x2, y2 = roi_coords
