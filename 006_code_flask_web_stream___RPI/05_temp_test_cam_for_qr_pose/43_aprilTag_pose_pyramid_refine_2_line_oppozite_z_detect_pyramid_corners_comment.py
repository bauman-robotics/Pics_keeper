...
DEBUG_PYRAMID = True # Включает всю отладку

# Или для тонкой настройки:
DEBUG_PYRAMID = True
DRAW_LINES = True
...
def detect_pyramid_corners(frame_gray, rvec, tvec, camera_matrix, dist_coeffs):
    ...
    Параметры:
        ...
        rvec - вектор поворота маркера (3D)
        tvec - вектор перемещения маркера (3D)
        camera_matrix - матрица камеры (внутренние параметры)
        dist_coeffs - коэффициенты дисторсии
    
    Возвращает:
        results - список результатов для каждой грани

    Для каждой грани возвращается словарь со следующей структурой:
    {
        'face': str,           # Название грани: 'FRONT', 'BACK', 'LEFT', 'RIGHT'
        'center_3d': ndarray,  # 3D координаты центра грани (в мм, в системе маркера)
        'corner_2d': ndarray,  # 2D координаты найденного угла на изображении [x, y] (или None)
        'proj_2d': ndarray,    # 2D проекция центра грани на изображении [x, y]
        'reproj_err': float,   # Ошибка репроекции в пикселях (разница между corner_2d и proj_2d)
        'visible': bool,       # Флаг видимости грани (обращена ли к камере)
        'confidence': float,   # Уверенность в детекции от 0.0 до 1.0
    }

    """
    results = []  # Список для хранения результатов по каждой грани
    
    # Получаем размеры изображения
    h, w = frame_gray.shape[:2]
    
    # Вычисляем расстояние до маркера (норма вектора перемещения)
    distance_m = float(np.linalg.norm(tvec))
    
    # Извлекаем фокусное расстояние камеры (не используется в текущей реализации)
    fx = camera_matrix[0, 0]

    # Преобразуем вектор поворота в матрицу поворота 3x3
    # Rodrigues - это формула Родрига для конверсии
    R, _ = cv2.Rodrigues(rvec)

    # ========== ОБРАБОТКА КАЖДОЙ ГРАНИ ПИРАМИДЫ ==========
    # PYRAMID_FACES - глобальная переменная со списком граней
    for face in PYRAMID_FACES:
        name = face['name']           # Название грани (FRONT, BACK, LEFT, RIGHT)
        center_3d = face['center_3d'] # Координаты центра грани в 3D (в мм)
        normal = face['normal']        # Вектор нормали грани

        # Проверяем, видна ли грань (обращена ли к камере)
        # is_face_visible вычисляет скалярное произведение нормали и направления взгляда
        visible, dot_val = is_face_visible(normal, rvec, tvec)

        # ========== ПРОЕКЦИЯ ЦЕНТРА ГРАНИ ==========
        # Переводим координаты из миллиметров в метры (делим на 1000)
        # reshape(1, 1, 3) - приводим к формату, который ожидает projectPoints
        center_m = (center_3d / 1000.0).reshape(1, 1, 3).astype(np.float32)
        
        # Проецируем 3D точку на 2D плоскость изображения
        # projectPoints использует параметры камеры для преобразования
        proj, _ = cv2.projectPoints(center_m, rvec, tvec, camera_matrix, dist_coeffs)
        px, py = proj[0][0]  # Координаты проекции центра грани

        # Базовый результат для этой грани (если не найдем угол)
        result_base = {
            'face': name,              # Название грани
            'center_3d': center_3d,    # 3D координаты центра
            'corner_2d': None,         # Найденный угол (пока None)
            'proj_2d': np.array([px, py]),  # Проекция центра на 2D
            'reproj_err': None,         # Ошибка репроекции (пока None)
            'visible': visible,         # Видимость грани
            'confidence': 0.0,          # Уверенность в детекции (0-1)
        }

        # Если грань не видна - сохраняем базовый результат и переходим к следующей
        if not visible:
            results.append(result_base)
            continue

        # ========== ПРОЕЦИРУЕМ ВСЕ 4 УГЛА ГРАНИ ==========
        # Размеры пирамиды в метрах (перевод из мм)
        half_top = PYRAMID_TOP_SIZE / 2.0 / 1000.0    # Половина верхнего основания
        half_base = PYRAMID_BASE_SIZE / 2.0 / 1000.0  # Половина нижнего основания
        h_pyr = PYRAMID_HEIGHT / 1000.0                # Высота пирамиды
        
        # Определяем координаты 4 углов для каждой грани
        # Все координаты в системе координат маркера (центр в основании пирамиды)
        # Порядок: два верхних угла (Z=0), затем два нижних угла (Z=высота)
        if name == 'FRONT':
            # Передняя грань: X от -half до +half, Y постоянно -half_top/-half_base
            vertices_3d = np.array([
                [-half_top, -half_top, 0.0],  # Левый верхний
                [ half_top, -half_top, 0.0],  # Правый верхний
                [ half_base, -half_base, h_pyr],  # Правый нижний
                [-half_base, -half_base, h_pyr]   # Левый нижний
            ], dtype=np.float32)
        elif name == 'BACK':
            # Задняя грань: X от -half до +half, Y постоянно +half_top/+half_base
            vertices_3d = np.array([
                [ half_top, half_top, 0.0],   # Правый верхний
                [-half_top, half_top, 0.0],   # Левый верхний
                [-half_base, half_base, h_pyr], # Левый нижний
            ...
            # Левая грань: Y от -half до +half, X постоянно -half_top/-half_base
            vertices_3d = np.array([
                [-half_top, -half_top, 0.0],  # Передний верхний
                [-half_top, half_top, 0.0],   # Задний верхний
                [-half_base, half_base, h_pyr], # Задний нижний
                [-half_base, -half_base, h_pyr]  # Передний нижний
            ], dtype=np.float32)
        else:  # RIGHT
            # Правая грань: Y от -half до +half, X постоянно +half_top/+half_base
            vertices_3d = np.array([
                [ half_top, half_top, 0.0],   # Задний верхний
                [ half_top, -half_top, 0.0],  # Передний верхний
                [ half_base, -half_base, h_pyr], # Передний нижний
        ...
        # Проецируем все 4 угла грани на изображение
        vertices_2d, _ = cv2.projectPoints(
            vertices_3d, rvec, tvec, camera_matrix, dist_coeffs
        )
        vertices_2d = vertices_2d.reshape(-1, 2)  # Преобразуем в массив точек
        
        # ========== ВЫДЕЛЕНИЕ ROI (ОБЛАСТИ ИНТЕРЕСА) ==========
        # Находим bounding box грани (минимальный прямоугольник, содержащий все углы)
        min_x = np.min(vertices_2d[:, 0])
        max_x = np.max(vertices_2d[:, 0])
        min_y = np.min(vertices_2d[:, 1])
        max_y = np.max(vertices_2d[:, 1])
        
        # Вычисляем размеры грани
        face_width = max_x - min_x
        face_height = max_y - min_y
        
        # Добавляем отступы (30% от размера грани) для надежности детекции
        ...
        # Проверяем, что ROI достаточно большой (минимум 40 пикселей)
        ...
        # ========== ОПРЕДЕЛЕНИЕ НАПРАВЛЕНИЙ ЛИНИЙ В 3D ==========
        # Для каждой грани определяем, какие оси соответствуют горизонтали и вертикали
        # Это нужно для фильтрации линий на изображении
        
        # Базовая ориентация в 3D (в системе координат маркера)
        if name == 'FRONT' or name == 'BACK':
            # Для FRONT/BACK: горизонталь вдоль оси X, вертикаль вдоль оси Z
            horizontal_dir_3d = np.array([1.0, 0.0, 0.0])  # Ось X
            vertical_dir_3d = np.array([0.0, 0.0, 1.0])    # Ось Z
            ...
            # Для LEFT/RIGHT: горизонталь вдоль оси Y, вертикаль вдоль оси Z
            horizontal_dir_3d = np.array([0.0, 1.0, 0.0])  # Ось Y
            vertical_dir_3d = np.array([0.0, 0.0, 1.0])    # Ось Z
            
            top_point_3d = center_3d / 1000.0  # Верх грани
            bottom_point_3d = center_3d / 1000.0 + np.array([0.0, 0.0, h_pyr])  # Низ грани

        # ========== ОПРЕДЕЛЕНИЕ ВЕРТИКАЛЬНОГО НАПРАВЛЕНИЯ НА ИЗОБРАЖЕНИИ ==========
        # Проецируем верхнюю и нижнюю точки грани на изображение
        ...
        # Вычисляем вектор вертикали на изображении (от верхней точки к нижней)
        ...
        roi_blur = cv2.GaussianBlur(roi, (5, 5), 0)  # Размытие для уменьшения шума
        edges = cv2.Canny(roi_blur, 50, 150, apertureSize=3)  # Детектор границ Кэнни
        
        # Поиск линий через вероятностное преобразование Хафа
        # Находит отрезки прямых линий на изображении
        lines = cv2.HoughLinesP(
            edges, 
            rho=1,                    # Шаг по расстоянию в пикселях
            theta=np.pi/180,           # Шаг по углу в радианах (1 градус)
            threshold=30,               # Минимальное количество пересечений для детекции линии
            minLineLength=roi.shape[1] // 4,  # Минимальная длина линии (1/4 ширины ROI)
            maxLineGap=10                # Максимальный разрыв в линии
        )
        
        # Проверяем, найдены ли линии и достаточно ли их
        if lines is None or len(lines) < 4:
            results.append(result_base)
            continue
        
        # ========== КЛАССИФИКАЦИЯ ЛИНИЙ ПО НАПРАВЛЕНИЮ ==========
        lines_list = lines[:, 0, :]  # Преобразуем в удобный формат
        
        # Списки для кандидатов в горизонтальные и вертикальные линии
        horizontal_candidates = []
        vertical_candidates = []
        
        # Анализируем каждую найденную линию
        for line in lines_list:
            ...
            if length < 10:  # Пропускаем слишком короткие линии
                continue
                
            # Нормализованный вектор направления
            dir_vec = np.array([dx, dy]) / length
            
            # Центр линии (для последующей сортировки)
            center_x = (x1_l + x2_l) / 2
            center_y = (y1_l + y2_l) / 2
            
            # ========== ВЫЧИСЛЕНИЕ ОЖИДАЕМЫХ НАПРАВЛЕНИЙ ==========
            # Горизонтальное направление в 2D - это вектор, перпендикулярный вертикали
            # Поворачиваем вертикальный вектор на 90 градусов
            horizontal_dir_2d = np.array([-vertical_dir_2d_observed[1], vertical_dir_2d_observed[0]])
            
            # Вычисляем сходство линии с горизонталью (скалярное произведение)
            # Чем ближе к 1, тем линия ближе к горизонтальному направлению
            h_similarity = abs(np.dot(dir_vec, horizontal_dir_2d))
            
            # Вычисляем сходство линии с вертикалью
            v_similarity = abs(np.dot(dir_vec, vertical_dir_2d_observed))
            
            # Классифицируем линию на основе сходства с ожидаемыми направлениями
            if h_similarity > 0.7:  # Порог для горизонтали (cos < 45 градусов)
                horizontal_candidates.append({
                    'line': line,                     # Координаты линии
                    'center': (center_x, center_y),   # Центр линии
                    'similarity': h_similarity         # Степень сходства
                })
        ...
        # Проверяем, что нашли хотя бы одну линию каждого типа
        ...
        # Сортируем кандидатов по близости к центру ROI
        # Для горизонтальных линий важна близость по Y (вертикальная координата)
        horizontal_candidates.sort(key=lambda c: abs(c['center'][1] - roi_center_y))
        # Для вертикальных линий важна близость по X (горизонтальная координата)
        vertical_candidates.sort(key=lambda c: abs(c['center'][0] - roi_center_x))
        
        # Берем лучшие (ближайшие к центру) линии
        best_horizontal = horizontal_candidates[0]['line']
        best_vertical = vertical_candidates[0]['line']

        # ========== НАХОЖДЕНИЕ ТОЧКИ ПЕРЕСЕЧЕНИЯ ==========
        # Распаковываем координаты линий
        x1_h, y1_h, x2_h, y2_h = best_horizontal
        x1_v, y1_v, x2_v, y2_v = best_vertical

        # Вычисляем параметры уравнений прямых в форме: a*x + b*y + c = 0
        ...
        # Вычисляем детерминант для решения системы уравнений
        det = a_h * b_v - a_v * b_h
        
        # Если детерминант близок к нулю - линии параллельны
        if abs(det) < 1e-6:
            results.append(result_base)
            continue

        # Находим точку пересечения методом Крамера
        cx_roi = (b_h * c_v - b_v * c_h) / det
        cy_roi = (c_h * a_v - c_v * a_h) / det

        # ========== ПРОВЕРКА КОРРЕКТНОСТИ ТОЧКИ ==========
        margin = 5  # Отступ от границ ROI
        roi_h, roi_w = roi.shape[:2]

        # Проверяем, что точка внутри ROI (с учетом отступа)
        ...
        # Подготавливаем точку для уточнения
        ...
        try:
            # Уточняем положение угла с субпиксельной точностью
            # Используем метод, который анализирует градиенты в окрестности точки
            corners_subpix = cv2.cornerSubPix(
                roi,
                candidate,
                winSize=(11, 11),        # Размер окна поиска
                zeroZone=(-1, -1),        # Нет зоны игнорирования
                criteria=(cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)
                # Критерий остановки: 30 итераций или точность 0.001
            )
            
            corner_roi = corners_subpix[0][0]  # Уточненные координаты в ROI
            
            # Проверяем результат уточнения
        ...
        # ========== ПЕРЕВОД КООРДИНАТ В ИСХОДНОЕ ИЗОБРАЖЕНИЕ ==========
        # Добавляем смещение ROI к координатам
        ...
        # Сравниваем найденный угол с проекцией центра грани
        reproj_err = float(np.linalg.norm(corner_full - np.array([px, py])))
        
        # Проверяем, что ошибка не слишком велика
        # Порог увеличивается с расстоянием до объекта
        ...
        # - количества найденных линий (чем больше, тем лучше)
        # - ошибки репроекции (чем меньше, тем лучше)
        confidence = min(1.0, 
                        (len(horizontal_candidates) + len(vertical_candidates)) / 10.0 *
                        (1.0 - reproj_err / (PYRAMID_REPROJ_THRESHOLD * 2)))
        
        # ========== СОХРАНЕНИЕ РЕЗУЛЬТАТА ==========
        result = result_base.copy()  # Копируем базовый результат
        result['corner_2d'] = corner_full      # Найденный угол
        result['reproj_err'] = reproj_err      # Ошибка репроекции
        result['confidence'] = max(0.3, confidence)  # Уверенность (минимум 0.3)
        
        results.append(result)
    
    return results  # Возвращаем результаты для всех граней
...
def draw_pyramid_corners(frame, pyramid_results, rvec, tvec, camera_matrix, dist_coeffs):
    """
    Отрисовывает на кадре проекции центров граней, найденные точки пересечения линий,
    а также детектированные линии на гранях (с поддержкой отладки).
    ...
    Макросы отладки (определите перед функцией):
        DRAW_LINES = True           - рисовать все найденные линии
        DRAW_CLASSIFIED = True      - рисовать классифицированные линии (красный/зеленый)
        DRAW_ROI = True             - рисовать области интереса
        DRAW_INTERSECTIONS = True   - рисовать точки пересечения
        DEBUG_TEXT = True           - выводить отладочный текст
    """
    ...
    # Можно также определить глобальные переменные для тонкой настройки
    # DRAW_LINES = globals().get('DEBUG_PYRAMID', False)
    # DRAW_CLASSIFIED = globals().get('DEBUG_PYRAMID', False)
    # и т.д.
    ...
    for r in pyramid_results:
        ...
        # ===== НАЙДЕННЫЙ ЦЕНТР ГРАНИ =====
        # Используем get() для совместимости с разными версиями
        center_2d = r.get('center_2d') or r.get('corner_2d')
