...
def detect_pyramid_corners(frame_gray, rvec, tvec, camera_matrix, dist_coeffs):
    """
    Детектирует углы пирамиды используя углы AprilTag как базу.
    С правильной геометрией для каждой грани.
    """
    ...
    # Проецируем все 4 угла AprilTag
    half = MARKER_SIZE / 2.0 / 1000.0
    tag_corners_3d = np.array([
        [-half, -half, 0.0],  # 0: левый верхний
        [ half, -half, 0.0],  # 1: правый верхний
        [ half,  half, 0.0],  # 2: правый нижний
        [-half,  half, 0.0]   # 3: левый нижний
    ], dtype=np.float32)
    
    tag_corners_2d, _ = cv2.projectPoints(
        tag_corners_3d, rvec, tvec, camera_matrix, dist_coeffs
    )
    tag_corners_2d = tag_corners_2d.reshape(-1, 2)
    
    # Вычисляем размер маркера в пикселях
    marker_width = np.linalg.norm(tag_corners_2d[1] - tag_corners_2d[0])
    marker_height = np.linalg.norm(tag_corners_2d[2] - tag_corners_2d[1])
    marker_diag = np.linalg.norm(tag_corners_2d[2] - tag_corners_2d[0])
    
    if DEBUG_PYRAMID:
        debug_frame = cv2.cvtColor(frame_gray, cv2.COLOR_GRAY2BGR)
        print(f"\n{'='*60}")
        print(f"🔍 PYRAMID CORNER DETECTION DEBUG (FIXED GEOMETRY) - Frame {h}x{w}")
        print(f"{'='*60}")
        print(f"📏 Distance to marker: {distance_m:.3f}m")
        print(f"📏 Marker in image: {marker_width:.1f}x{marker_height:.1f}px, diag={marker_diag:.1f}px")
        
        # Рисуем углы AprilTag
        for i, corner in enumerate(tag_corners_2d):
            color = (0, 255, 0)
            cv2.circle(debug_frame, (int(corner[0]), int(corner[1])), 8, color, -1)
            cv2.putText(debug_frame, f"Tag{i}", 
                       (int(corner[0])+15, int(corner[1])-15),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)
    
    # Вычисляем направления осей
    R, _ = cv2.Rodrigues(rvec)
    
    # Оси маркера в системе камеры
    x_axis = R[:, 0]  # красный
    y_axis = R[:, 1]  # зеленый
    z_axis = R[:, 2]  # синий
    ...
    # Для каждой грани определяем смещение в 3D
    face_offsets_3d = {
        'FRONT': np.array([0.0, -half_top, 0.0]),     # Вперед по Y
        'BACK': np.array([0.0, half_top, 0.0]),       # Назад по Y
        'LEFT': np.array([-half_top, 0.0, 0.0]),      # Влево по X
        'RIGHT': np.array([half_top, 0.0, 0.0])       # Вправо по X
    }
    ...
    for face_idx, face in enumerate(PYRAMID_FACES):
        ...
        # Получаем 3D смещение для этой грани
        offset_3d = face_offsets_3d[name]
        
        # Проецируем смещение, чтобы получить 2D вектор
        offset_point_3d = offset_3d.reshape(1, 1, 3).astype(np.float32)
        offset_point_2d, _ = cv2.projectPoints(
            offset_point_3d, rvec, tvec, camera_matrix, dist_coeffs
        )
        offset_point_2d = offset_point_2d[0][0]
        
        # Проецируем центр маркера
        ...
        # Вычисляем 2D вектор смещения
        offset_2d = offset_point_2d - center_2d
        
        # Находим соответствующий угол маркера для базы
        if name == 'FRONT':
            base_corner = tag_corners_2d[0]  # левый верхний
        elif name == 'RIGHT':
            base_corner = tag_corners_2d[1]  # правый верхний
        elif name == 'BACK':
            base_corner = tag_corners_2d[2]  # правый нижний
        else:  # LEFT
            base_corner = tag_corners_2d[3]  # левый нижний
        
        # Предполагаемое положение угла пирамиды
        # Смещаемся от угла маркера в направлении offset_2d
        estimated_corner = base_corner + offset_2d * 0.5  # Половина смещения
        
        if DEBUG_PYRAMID:
            print(f"   👁️ Visible: {visible}")
            print(f"   📍 Угол маркера: ({base_corner[0]:.1f}, {base_corner[1]:.1f})")
            print(f"   📐 Вектор смещения: ({offset_2d[0]:.1f}, {offset_2d[1]:.1f})px")
            print(f"   📍 Предполагаемый угол: ({estimated_corner[0]:.1f}, {estimated_corner[1]:.1f})")
            cv2.circle(debug_frame, (int(estimated_corner[0]), int(estimated_corner[1])), 
                      8, (255, 255, 0), -1)
            cv2.arrowedLine(debug_frame, 
                          (int(base_corner[0]), int(base_corner[1])),
                          (int(estimated_corner[0]), int(estimated_corner[1])),
                          (255, 255, 0), 2)
        
        if not visible:
            result = {
                'face': name,
                'corner_3d': np.array([0,0,0]),
                'corner_2d': None,
                'proj_corner': estimated_corner,
                'reproj_err': None,
                'visible': visible,
                'confidence': 0.0
            }
        ...
        # ========== УВЕЛИЧЕННОЕ ROI ==========
        roi_size = int(marker_diag * 0.3)  # 30% от диагонали
        roi_size = max(80, min(150, roi_size))
        
        x1 = max(int(estimated_corner[0] - roi_size/2), 0)
        y1 = max(int(estimated_corner[1] - roi_size/2), 0)
        x2 = min(int(estimated_corner[0] + roi_size/2), w)
        y2 = min(int(estimated_corner[1] + roi_size/2), h)
        ...
        if DEBUG_PYRAMID:
            ...
            cv2.putText(debug_frame, name, (x1, y1-10), 
        ...
        global_corners = corners + np.array([x1, y1])
        
        # Находим ближайший к предполагаемому углу
        min_dist = float('inf')
        best_corner = None
        
        for corner in global_corners:
            dist = np.sqrt((corner[0] - estimated_corner[0])**2 + 
                          (corner[1] - estimated_corner[1])**2)
            if dist < min_dist:
                min_dist = dist
                best_corner = corner
        ...
        if DEBUG_PYRAMID:
            print(f"   ✅ Найден угол: ({best_corner[0]:.1f}, {best_corner[1]:.1f})")
            print(f"   📏 Ошибка: {reproj_err:.1f}px")
            cv2.circle(debug_frame, (int(best_corner[0]), int(best_corner[1])), 
                      8, (0, 255, 0), -1)
            cv2.line(debug_frame, 
                    (int(estimated_corner[0]), int(estimated_corner[1])), 
                    (int(best_corner[0]), int(best_corner[1])), 
                    (255, 255, 255), 2)
        
        # Динамический порог
        max_allowed_error = marker_diag * 0.1  # 10% от диагонали
        ...
        if reproj_err > max_allowed_error:
            ...
            confidence = 1.0 - min(1.0, reproj_err / max_allowed_error)
            confidence = max(0.3, min(1.0, confidence))
        
        # TODO: вычислить реальные 3D координаты
        corner_3d = np.array([0,0,0])
        
        result = {
            'face': name,
            'corner_3d': corner_3d,
            'corner_2d': np.array(best_corner),
            'proj_corner': estimated_corner,
    ...
    if DEBUG_PYRAMID:
        found = sum(1 for r in results if r['corner_2d'] is not None)
        print(f"\n{'='*60}")
        print(f"📊 ИТОГОВАЯ СТАТИСТИКА:")
        print(f"   Найдено углов: {found}/4")
        print(f"   Размер маркера: {marker_diag:.1f}px диагональ")
        for r in results:
            if r['corner_2d'] is not None:
                print(f"   {r['face']}: ошибка {r['reproj_err']:.1f}px, уверенность {r['confidence']:.2f}")
        print(f"{'='*60}")
        
        cv2.imshow('Pyramid Debug - Fixed Geometry', debug_frame)
        cv2.waitKey(1)
